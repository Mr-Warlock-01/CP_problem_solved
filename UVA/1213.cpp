...
void prime_generate(){
    vector<bool>v(1121,1);
    for(int i=2; (i*i)<1121; i++){
        if(v[i]){
            for(int j=i*i; j<1121; j+=i){v[j]=0;}
        }
    }
    for(int i=2; i<1121; i++){
        if(v[i]){prime_number.pb(i);}
    }
}


int xxx(int num, int cnt, int pos, vector<vector<vector<int>>>&dp){
    if((num==0) && (cnt==0) && (pos==-1)){return 1;}
    if((num<0) || (cnt<0) || (pos<0)){return 0;}
    if(dp[num][cnt][pos]!=-1){return dp[num][cnt][pos];}
    return dp[num][cnt][pos] = xxx(num, cnt, pos-1, dp) + xxx(num-prime_number[pos], cnt-1,pos-1, dp); 
}


void pikachu(int T){         ///MAIN  (DRINK WATER)
    vector<vector<vector<int>>>dp(1121, vector<vector<int>>(15,vector<int>(prime_number.size()+1, -1)));

    while(1){
        int n,k;    cin>>n>>k;
        if((n==0) && (k==0)){break;}
        cout<<xxx(n,k,prime_number.size()-1, dp)<<endl;
    }
return;
}
