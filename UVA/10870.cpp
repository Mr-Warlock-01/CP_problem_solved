...
Matrix mul(Matrix &A, Matrix &B, int M_sz) {
    Matrix res = {};
...
Matrix matrix_pow(Matrix &mat, int n, int M_sz) {
    ...
    while(n){
        if(n&1){res = mul(res, mat, M_sz);}
        if(n>0){mat = mul(mat, mat, M_sz);}
        n>>=1;
    }
    return res;
}
...
void solve(int T){      //DRINK WATER
    while(1){
        ...
        if(n<=d){cout<<f[n-1]%M<<endl;}
            ...
            Seq = matrix_pow(Seq, n-d, d);
            Matrix res = mul(Seq, Pat, d);
            cout<<res[0][0]<<endl;
        }
    }
}   
