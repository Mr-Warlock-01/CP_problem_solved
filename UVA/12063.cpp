...
int xxx(int pos, int one, int sum, vector<vector<vector<int>>>&dp){
    if(pos==n){
        if((sum%k==0) && ((pos-one)==one)){return 1;}
        return 0;
    }
    if(dp[pos][one][sum]!=-1){return dp[pos][one][sum];}

    int a=xxx(pos+1, one+1, (powMod(2,pos,k)+sum)%k ,dp);
    int b=0;
    if(pos!=(n-1)){b=xxx(pos+1, one, sum ,dp);}
    return dp[pos][one][sum]=a+b;
}
