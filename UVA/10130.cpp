...
int xxx(int pos, int weight, vector<pair<int,int>>&v, vector<vector<int>>&dp){
    if(weight<0){return LLMin;}
    if(pos==n){return 0;}
    if(dp[pos][weight]!=-1){return dp[pos][weight];}

    int a=xxx(pos+1, weight, v, dp);
    int b=xxx(pos+1, weight-v[pos].ss, v, dp)+v[pos].ff;
    return dp[pos][weight]=max(a,b);
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    int m;  cin>>m;
    vector<int>person(m);   cin>>person;
    
    int ans=0;
    for(int i=0; i<m; i++){
        ans+=xxx(0,person[i],v,dp);
    }
