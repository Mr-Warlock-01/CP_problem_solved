...
void dfs(int x, vector<int>graph[], vector<bool>&visit, string &s){
    visit[x]=1;
    bool flag=1;
    for(int i=0; i<graph[x].size(); i++){
        if(!visit[graph[x][i]]){
            dfs(graph[x][i], graph, visit, s);
            flag=0;
        }
    }
    if(flag){
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<n; i++){if(s[i]=='?'){tot_oz++;}}
    dfs(1, graph, visit, s);

    int ans=0;
    if(s[0]=='0'){ans=one+((o_z+1)/2);}
    else if(s[0]=='1'){ans=zero+((o_z+1)/2);}
    else{
        ans=max(zero,one)+(o_z/2);
        if((zero==one) && (tot_oz-o_z)%2==0){
            ans=max(zero,one)+((o_z+1)/2);
        }
    }
    cout<<ans<<endl;  
return;
