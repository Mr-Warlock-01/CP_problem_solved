...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    if(n==2){cout<<(10*v[0])+v[1]<<endl;}
    else if(n==3){
        int a=((10*v[0])+v[1])*v[2];
        int b=((10*v[0])+v[1])+v[2];
        int c=((10*v[1])+v[2])*v[0];
        int d=((10*v[1])+v[2])+v[0];
        ...
        for(int i=1; i<n; i++){
            ...
            ok=min(ok, (10*v[i-1])+v[i]);
        }

        bool flag=0;
        for(int i=0; i<(n-1); i++){
            int lol=(10*v[i])+v[i+1];
            if(flag==0 && lol==ok){
                flag=1; ans1.pb(lol);    i++;
            }
            else{ans1.pb(v[i]);}
        }
        if((ans1.size()+1)!=v.size()){ans1.pb(v[n-1]);}
        ...
        for(int i=0; i<(n-1); i++){
            int lol=(10*v[i])+v[i+1];
            if(flag==0 && lol==ok){
                flag=1; ans2.pb(lol);    i++;
            }
            else{ans2.pb(v[i]);}
        }
        if((ans2.size()+1)!=v.size()){ans2.pb(v[n-1]);}
        ...
        for(int i=0; i<(n-1); i++){
            int lol=(10*v[i])+v[i+1];
            if(flag==0 && lol==ok){
                flag=1; ans3.pb(lol);    i++;
            }
            else{ans3.pb(v[i]);}
        }
        if((ans3.size()+1)!=v.size()){ans3.pb(v[n-1]);}


        int ANS3=0;
        for(int i=0; i<ans3.size(); i++){
            if(ans3[i]==1){continue;}
            ANS3+=ans3[i];
        }
        ...
        flag=0;
        for(int i=0; i<ans1.size();i++){
            if(ans1[i]>9){flag=1; break;}
        }
        if(flag==0){ANS1=LONG_MAX;}

        flag=0;
        for(int i=0; i<ans2.size();i++){
            if(ans2[i]>9){flag=1; break;}
        }
        if(flag==0){ANS2=LONG_MAX;}



        flag=0;
        for(int i=0; i<ans3.size();i++){
            if(ans3[i]>9){flag=1; break;}
        }
        if(flag==0){ANS3=LONG_MAX;}
