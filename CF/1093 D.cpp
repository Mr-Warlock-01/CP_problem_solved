...
void dfs(int x, vector<int>graph[], vector<int>&visit, bool flag){
    if(flag==0){a++; visit[x]=1;}
    else{b++; visit[x]=2;}
    for(int i=0; i<graph[x].size(); i++){
        if(visit[graph[x][i]]==0){
            dfs(graph[x][i], graph, visit, flag^1);
        }
        else if(flag==1 && visit[graph[x][i]]==2){possible=0;}
        else if(flag==0 && visit[graph[x][i]]==1){possible=0;}
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=1; i<=n; i++){
        if(!visit[i]){
            a=0; b=0;
            dfs(i, graph, visit, 0);
            ans= (ans* ((powMod(2, a, modulo)+powMod(2, b, modulo))%modulo))% modulo;
        }
    }
    if(!possible){ans=0;}
    cout<<ans<<endl;
