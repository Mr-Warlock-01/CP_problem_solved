...
vector<vector<int>>treee(4*N,vector<int>(26));
int ans[26];
string s; 


void build(int node,int st, int en){
    if(st==en){treee[node][s[st]-'a']++; return;}
    int mid=((en-st)/2)+st;
    build(2*node, st, mid);
    build(2*node+1, mid+1, en);
    for(int i=0; i<26; i++){
        treee[node][i]=treee[2*node][i]+treee[2*node+1][i];
    }
return;
}
...
void query(int node, int st, int en, int l, int r){
    if(st>r || en<l){return;}
        ...
        for(int i=0; i<26; i++){ans[i]+=treee[node][i];}
        return ;
    }
    int mid=((en-st)/2)+st;
...
void update(int node, int st, int en, int ind, char c){
    treee[node][s[ind]-'a']--;
    if(st==en){treee[node][c-'a']++;   s[ind]=c; return;}
    int mid=((en-st)/2)+st;
    if(st<=ind && mid>=ind){update(2*node,st, mid, ind, c);}
    else{update(2*node+1, mid+1, en, ind, c);}
    treee[node][c-'a']++; 
    return;
}
