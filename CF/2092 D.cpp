...
void solve(int T){
    ...
    if(l_cnt==n || t_cnt==n || i_cnt==n){IMP; return;}

    vector<int>v={l_cnt, t_cnt, i_cnt};
    sort(all(v));
    ...
    int zzz=v[2]+dif;
    int l_need=zzz-l_cnt;
    int t_need=zzz-t_cnt;
    int i_need=zzz-i_cnt;
    int need= l_need+ t_need + i_need;
    stack<char>st;
    ...
    while(!st.empty()){
        if(need==0){break;}
        else if(c=='L' && st.top()=='I' & t_need>0){
            st.push('T');
            t_need--;   need--;
            ans.pb(idx);
        }
        else if(c=='L' && st.top()=='T' & i_need>0){
            st.push('I');
            i_need--;   need--;
            ans.pb(idx);
        }
        else if(c=='I' && st.top()=='L' & t_need>0){
            st.push('T');
            t_need--;   need--;
            ans.pb(idx);
        }
        else if(c=='I' && st.top()=='T' & l_need>0){
            st.push('L');
            l_need--;   need--;
            ans.pb(idx);
        }
        else if(c=='T' && st.top()=='L' & i_need>0){
            st.push('I');
            i_need--;   need--;
            ans.pb(idx);
        }
        else if(c=='T' && st.top()=='I' & l_need>0){
            st.push('L');
            l_need--;   need--;
            ans.pb(idx);
        }
