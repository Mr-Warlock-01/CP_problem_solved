...
vector<int>v(3);
pair<int,int> two_max(int a, int b, int c){ 
    v[0]=a; v[1]=b; v[2]=c;
    sort(rall(v));
    return {v[0],v[1]};
}
...
void dfs2(int x, int p){
    ...
    for(int i=0; i<graph[x].size(); i++){
        if(graph[x][i]!=p){
            sz++;
            dfs2(graph[x][i], x);
            ok=two_max(ok.ff,ok.ss,dp[graph[x][i]]);
        }
    }
    int z=0;
         if(ok.ff==0){z=graph[x].size();}
    else if(ok.ss==0){z=graph[x].size()+ ok.ff -1;}
    else             {z=graph[x].size()+ ok.ff + ok.ss - 2;}
    ans=max(ans, z);
return;
}
