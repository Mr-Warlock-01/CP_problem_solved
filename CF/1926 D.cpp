...
int power[33]={1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072,
    262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216, 33554432, 67108864, 134217728, 268435456, 536870912, 1073741824, 2147483648, 429496729};

int revert(int a){
    int x=a;
    int ans=0;
    for(int i=0; x>0;i++){
        if(x%2==0){ans+=power[i];}
        x/=2;
    }
    return ans;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    map<int,int>m;
    int ans=0;
    for(int i=0; i<n; i++){ if(v[i]<=1073741823){m[v[i]]++; ans++;}}
