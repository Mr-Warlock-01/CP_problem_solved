...
int BS_left_most(vector<pair<int,int>>&v, int x){
    int l=0, r=v.size()-1;
    int ans;
    while(l<=r){
        int mid=((r-l)/2)+l;
        if(v[mid].ff==x){return mid;}
        else if(v[mid].ff>x){ans=mid; r=mid-1;}
        else{l=mid+1;}
    }
    return ans;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<(n-1); i++){
        if(v[i]!=v[i+1]){indx.pb({i, v[i]});}
    }
    indx.pb({n-1, v[n-1]});

    while(q--){
        int l,r,x;  cin>>l>>r>>x;
        int ok=BS_left_most(indx, l-1);
        
        if(indx[ok].ss!=x){cout<<l<<endl;}
        else if(indx[ok].ff<(r-1)){cout<<(indx[ok].ff+2)<<endl;}
        else{cout<<"-1"<<endl;}
    }
}
