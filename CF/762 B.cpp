...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    while(1){
        if(a==0 || usb.empty()==1){break;}
        eq++;   a--;
        cost+=usb.top();
        usb.pop();
    }
    while(1){
        if(b==0 || ps.empty()==1){break;}
        eq++;   b--;
        cost+=ps.top();
        ps.pop();
    }
    while(1){
        if(c==0 || (usb.empty()==1 && ps.empty()==1)){break;}
        if(usb.empty()==1){cost+=ps.top(); ps.pop();}
        else if(ps.empty()==1){cost+=usb.top();    usb.pop();}
        else{
            if(usb.top()<ps.top()){cost+=usb.top();    usb.pop();}
            else{cost+=ps.top(); ps.pop();}
        }
        eq++;   c--;
    }
    cout<<eq<<" "<<cost;
