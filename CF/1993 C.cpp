...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    int x=2*k;
    int on_max=-1,on_min=LLMax;
    int off_max=-1,off_min=LLMax;

    for(int i=0; i<n; i++){
        int ok=(v[i]-v[0])%x;
        if(ok<k){
            on_max=max(on_max, ok);
            on_min=min(on_min, ok);
        }
        else{
            off_max=max(off_max, ok);
            off_min=min(off_min, ok);
        }
    }

    int haha=x-off_min;
     
    if((k-on_max)<=haha  ){cout<<"-1"<<endl;}
    else{
