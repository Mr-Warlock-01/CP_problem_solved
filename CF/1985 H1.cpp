...
void bfs(int zzz, int yy, int xx, vector<vector<char>>&v,  vector<vector<bool>>&visit, vector<vector<pair<int,int>>>&node_reach, int n, int m){
    vector<pair<int ,int>>store;
    queue<pair<int,int>>qq;     //y//x
    qq.push({yy,xx});   store.pb({yy,xx});  visit[yy][xx]=1;
    while(!qq.empty()){
        int y=qq.front().ff;
        int x=qq.front().ss;
        qq.pop();

        if((x>0) && (visit[y][x-1]==0) && (v[y][x-1]=='#')){
            qq.push({y, x-1});  store.pb({y, x-1});     visit[y][x-1]=1;
        }
        if(((x+1)<m) && (visit[y][x+1]==0) && (v[y][x+1]=='#')){
            qq.push({y, x+1});  store.pb({y, x+1});     visit[y][x+1]=1;
        }
        if((y>0) && (visit[y-1][x]==0) && (v[y-1][x]=='#')){
            qq.push({y-1, x});  store.pb({y-1, x});     visit[y-1][x]=1;
        }
        if(((y+1)<n) && (visit[y+1][x]==0) && (v[y+1][x]=='#')){
            qq.push({y+1, x});  store.pb({y+1, x});     visit[y+1][x]=1;
        }
    }
    for(int i=0; i<store.size(); i++){
        node_reach[store[i].ff][store[i].ss]={store.size(), zzz};
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n,m;    cin>>n>>m;
    vector<vector<char>>v(n,vector<char>(m));      cin>>v;
    vector<vector<bool>>visit(n,vector<bool>(m,0));
    vector<vector<pair<int,int>>>node_reach(n,vector<pair<int,int>>(m));        //node_count, dif_flag
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            if((v[i][j]=='#') && (visit[i][j]==0)){
                zzz++;
                bfs(zzz, i, j, v, visit, node_reach, n, m);
            }
        }
    }
    ...
    for(int i=0; i<n; i++){
        ...
        for(int j=0; j<m; j++){
            if(v[i][j]=='#'){
                if(os.order_of_key(node_reach[i][j].ss) ==  os.order_of_key(node_reach[i][j].ss+1)){
                        cou+=node_reach[i][j].ff;
                        os.insert(node_reach[i][j].ss);
                    }
            }
            else{cou++;}
        }
        ...
        if(i>0){
            for(int j=0; j<m; j++){
                if(v[i-1][j]=='#'){
                    if(os.order_of_key(node_reach[i-1][j].ss) ==  os.order_of_key(node_reach[i-1][j].ss+1)){
                        cou+=node_reach[i-1][j].ff;
                        os.insert(node_reach[i-1][j].ss);
                    }
                }
            }
        }
    ...
    for(int i=0; i<m; i++){
        ...
        for(int j=0; j<n; j++){
            if(v[j][i]=='#'){
                if(os.order_of_key(node_reach[j][i].ss) ==  os.order_of_key(node_reach[j][i].ss+1)){
                        cou+=node_reach[j][i].ff;
                        os.insert(node_reach[j][i].ss);
                    }
            }
            else{cou++;}
        }
        ...
        if(i>0){
            for(int j=0; j<n; j++){
                if(v[j][i-1]=='#'){
                    if(os.order_of_key(node_reach[j][i-1].ss) ==  os.order_of_key(node_reach[j][i-1].ss+1)){
                        cou+=node_reach[j][i-1].ff;
                        os.insert(node_reach[j][i-1].ss);
                    }
                }
            }
        }
