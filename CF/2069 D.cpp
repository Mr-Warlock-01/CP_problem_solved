...
bool xxx(int l, int r, int sz){
    bool flag1=1,flag2=1;
    int for_l_right_idx = l+sz-1;
    int for_r_left_idx  = r-sz+1;
    if(for_l_right_idx<for_r_left_idx){
        flag2=0;
        for(int i=0; i<26; i++){
            int cnt_1=arr[for_l_right_idx][i]-arr[l-1][i];
            int cnt_2=arr[r][i]-arr[for_r_left_idx-1][i];
            if(cnt_1!=cnt_2){flag1=0;}
        }
        for(int i=for_l_right_idx+1, j=for_r_left_idx-1; i<j; i++, j--){
            if(s[i]!=s[j]){flag1=0;}
        }
    }
    else{
        for(int i=0; i<26; i++){
            int cnt_1=arr[for_l_right_idx][i]-arr[l-1][i];
            int cnt_2=arr[r][i]-arr[for_l_right_idx][i];
            if(cnt_1<cnt_2){flag1=0;}
        }
        for(int i=0; i<26; i++){
            int cnt_1=arr[for_r_left_idx-1][i]-arr[l-1][i];
            int cnt_2=arr[r][i]-arr[for_r_left_idx-1][i];
            if(cnt_1>cnt_2){flag2=0;}
        }
    }
    return (flag1 | flag2);
}
...
void solve(int T){            ///   (DRINK WATER)
    cin>>s;
    s='a'+s;        
    s.pb('a');  //so that dont have to handle outside index
    n=s.size();
    ...
    int l=1,r=miss_match_r - miss_match_l+1;
    int ans=n; 
    while(l<=r){
        int mid=(l+r)/2;
        if(xxx(miss_match_l, miss_match_r, mid)){ans=mid; r=mid-1;}
        else{l=mid+1;}
    }
    cout<<ans<<endl;
}
