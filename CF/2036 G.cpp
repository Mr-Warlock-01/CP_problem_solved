...
ll xor_p(ll aa, ll bb){
    cout<<"xor "<<aa<<" "<<bb<<endl;
    ll ans; cin>>ans;
    return ans;
}

void search_ab(ll l, ll r){
    while(l<=r){
        ll mid=l+((r-l)/2);
        ll check=xor_p(l,mid);
        if(check==0){l=mid+1;}
        else if(check==ab_xor){r=mid;}
        else{
            a=check;
            b=ab_xor^a;
            break;
