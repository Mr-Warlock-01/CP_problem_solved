...
ll arithmatic_sum(ll a, ll n){
    if(n%2){return n*(((2*a)-n+1)/2);}
    else{ return ((2*a)-n+1)*(n/2);}
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    while(q--){
        ...
        while(l<=r){
            ...
            int a=sum[l_mid]-sum[x-1];
            int b=sum[r_mid]-sum[x-1];

            ll aaa=arithmatic_sum(c,a);
            ll bbb=arithmatic_sum(c,b);

            if(a==c){ans=l_mid; break;}
            if(b==c){ans=r_mid; break;}
            if((a<c) && (b<c)){
                if(bbb>flag){ans=r_mid;    flag=bbb;}
                else if(bbb==flag){ans=min(ans,r_mid);}
                l=r_mid+1;
            }
            else if((a>c) && (b>c)){
                if(aaa>flag){ans=l_mid;   flag=aaa;}
                else if(aaa==flag){ans=min(ans,l_mid);}
                r=l_mid-1;
            }
            else{
                if(aaa>flag){ans=l_mid;   flag=aaa;}
