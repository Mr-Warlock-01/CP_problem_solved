...
ll n,m,D, mx_d;
pair<ll,ll>no_jump[N];        //i n
pair<ll,ll>power_up[N];       //j m
...
void solve(int T){            ///   (DRINK WATER)
    ...
    mx_d=0;
    for(int i=0; i<n; i++){
        cin>>no_jump[i].ff>>no_jump[i].ss;
        mx_d=max(mx_d,no_jump[i].ss-no_jump[i].ff+1);
    }

    for(int i=0; i<m; i++){
        cin>>power_up[i].ff>>power_up[i].ss;
    }
    power_up[m].ff=1110000000;   power_up[m].ss=0;
    ...
    while(1){
        ...
        if(mx_d<power){break;}
        if(i>=n){break;}
        if(no_jump[i].ff<power_up[j].ff){
            ll z=no_jump[i].ss-no_jump[i].ff+1;
            while(power<=z){
                if(pq.empty()){ans=-1; break;}
                power+=pq.top();    pq.pop();   ans++;
            }
            i++;
        }
        else{
            pq.push(power_up[j].ss);    j++;
        }
    }
    cout<<ans<<endl;
return;
