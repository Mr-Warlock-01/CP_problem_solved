...
void pikachu(){         ///MAIN  (DRINK WATER)
    int n,k;    cin>>n>>k;
    vector<int> l(n),r(n),ans;  cin>>l; cin>>r;
    map<int,int>m1,m2;

    for(int i=0; i<n; i++){m1[l[i]]++;}
    for(int i=0; i<n; i++){m2[r[i]]++;}

    int flag1=2*k, flag2=2*k;

    for(int i=0; i<n && flag1!=0; i++){
        if(m1[l[i]]>1){m1[l[i]]-=2;flag1-=2;  cout<<l[i]<<" "<<l[i]<<" ";}
    }

    for(int i=0; i<n && flag1!=0; i++){
        if(m1[l[i]]==1){m1[l[i]]--;  flag1--;  cout<<l[i]<<" ";     ans.push_back(l[i]);}
    }
    cout<<endl;

    for(int i=0; i<n && flag2!=0; i++){
        if(m2[r[i]]>1){m2[r[i]]-=2;flag2-=2;  cout<<r[i]<<" "<<r[i]<<" ";}
    }
