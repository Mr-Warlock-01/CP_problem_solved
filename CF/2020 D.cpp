...
int find_set(int x, int i){
    if(x==parent[i][x]){return x;}
    return parent[i][x]= find_set(parent[i][x], i);
}
...
void union_sets(int a, int b, int i){
    ...
    b=find_set(b, i);
    if(a==b){return;}
    if(dsu_size[i][a]<dsu_size[i][b]){swap(a,b);}
    parent[i][b]=a;
    dsu_size[i][a]+=dsu_size[i][b];
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    while(m--){
        int a,d,k;  cin>>a>>d>>k;
        int z=skip_cnt[d][a+(d*k)];
        for(int i=0; i<=k; i++){
            int node=a+(d*i);
            int haha=skip_cnt[d][node];
            union_sets(a, node, d);
            skip_cnt[d][node]=max(skip_cnt[d][node], z+k-i);
            i+=haha;
        }
    }
