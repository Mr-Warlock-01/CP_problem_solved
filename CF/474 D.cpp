...
int xxx(int n, vector<int>&dp){
    if(n==0){return 1;}
    if(dp[n]!=-1){return dp[n];}
    int ans=0;
    if(n>=k){ans=xxx(n-k, dp);}
    ans+=xxx(n-1, dp);
    return dp[n]=ans%MOD;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    vector<int>dp(100002, -1);  dp[0]=0;
    vector<int>pre_sum(100002);

    int q;    cin>>q>>k;

    for(int i=1; i<100001; i++){pre_sum[i]=(pre_sum[i-1]+xxx(i,dp))%MOD;}

    while(q--){
        int l,r;    cin>>l>>r;
        int ans=(pre_sum[r]-pre_sum[l-1]+MOD)%MOD;
        cout<<ans<<endl;
    }
}
