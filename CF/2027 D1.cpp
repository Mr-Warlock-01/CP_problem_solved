...
const int N=300005;
ll n,m;    
ll a[N],b[N],v[N],pre_sum[N];  

int BS(int i, int pos){
    int ans=-1;
    int l=i+1, r=n;
    while(l<=r){
        int mid=(l+r)/2;
        ll sum=pre_sum[mid]-pre_sum[i];
        if(sum<=b[pos]){ans=mid;  l=mid+1;}
        else{r=mid-1;}
    }
    return ans;
}
...
ll xxx(int i, int j, vector<vector<ll>>&dp){
    if(i>=n){return 0;}
    if(j>=m){return LLMax;}
    if(dp[i][j]!=-1){return dp[i][j];}

    ll ans=LLMax;
    if(b[j]>=v[i]){ans=min(ans, xxx(i, j+1, dp));}
    ll sum=0;
    ll z=BS(i,j);
    if(z!=-1)ans=min(ans, xxx(z, j, dp)+m-j-1);
    return dp[i][j]=ans;
}
