...
int nth(int node, int st, int en, int cnt){
    if(st==en){return st;}
    int mid=((en-st)/2)+st;
    if(treee[2*node]>=cnt){
        return nth(2*node, st, mid, cnt);
    }
    return nth(2*node+1, mid+ 1, en, cnt-treee[2*node]);
}


void update(int node, int st, int en, int ind){
    if(st==en){treee[node]=0;  return;}
    int mid=((en-st)/2)+st;
...
void solve(int T){
    ...
    for(int i=k-1; i>=0; i--){
        int cnt=query(1,1,n, 1, v[i]);
        while(cnt!=0){
            int element=nth(1, 1, n,cnt);
            ans.pb(element);
            update(1,1,n,element);
            cnt--;
        }
    }
    int cnt=query(1,1,n, 1, n);
    while(cnt!=0){
        int element=nth(1, 1, n,cnt);
        ans.pb(element);
        update(1,1,n,element);
