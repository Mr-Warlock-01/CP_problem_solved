...
int xxx(vector<pair<double, int>>&v, int d){
    int l=0, r=v.size()-1 ,mid;
    int x=v.size()-1;
    while(l<=r){
        mid=((r-l)/2)+l;
        if(v[mid].ff==d){return v[mid].ss;}
        else if(v[mid].ff>d){ x=min(x,mid); r=mid-1;}
        else{ l=mid+1;}
    }
    //db(x);
    double f_d= v[x].ff-v[x-1].ff;
    int s_d= v[x].ss-v[x-1].ss;
    return floor (v[x-1].ss +( (s_d* (d-v[x-1].ff) )/f_d) );
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n,k,q; cin>>n>>k>>q;
    vector<pair<double,int>>v(k+1);
    for(int i=1; i<(k+1); i++){cin>>v[i].ff;}
    for(int i=1; i<(k+1); i++){cin>>v[i].ss;}
