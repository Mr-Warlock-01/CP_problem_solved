...
void dfs(int x, int flag, vector<int>graph[], vector<bool>&visit, vector<bool>&zero_one, vector<int>&in, vector<int>&out, vector<int>&v, vector<vector<int>>&arr){
    timer++;   visit[x]=1;   zero_one[x]=flag;   arr[0][timer]=arr[1][timer]=v[x];   in[x]=timer;   
    for(int i=0; i<graph[x].size(); i++){
        if(!visit[graph[x][i]]){
            dfs(graph[x][i],flag^1, graph, visit, zero_one, in, out, v, arr);
        }
    }
    out[x]=timer;
return;
...
void build(vector<int>&arr, vector<int>&tree, int node, int st, int en){
...
int lazy_query(vector<int>&tree, vector<int>&lazy, int node, int st, int en, int l, int r){
    if(lazy[node]!=0){
    ...
    int mid=((en-st)/2)+st;
    int q1= lazy_query(tree, lazy, 2*node, st, mid, l, r);
    int q2= lazy_query(tree, lazy, 2*node+1, mid+ 1, en, l, r);
    return q1+q2;
}


void lazy_update(vector<int>&tree, vector<int>&lazy, int node, int st, int en, int l, int r, int val){
    if(lazy[node]!=0){
        tree[node]+=((en-st+1)*lazy[node]);
        if(st!=en){ lazy[2*node]+=lazy[node];   lazy[2*node+1]+=lazy[node];}
        lazy[node]=0;
    ...
    int mid=((en-st)/2)+st;
    lazy_update(tree, lazy, 2*node, st, mid, l, r, val);
    lazy_update(tree, lazy, 2*node+1, mid+1, en, l, r, val);
    tree[node]=tree[2*node] + tree[2*node+1];
    return;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<bool>visit(n+1),zero_one(n+1);
    vector<vector<int>>arr(2, vector<int>(n+1));
    vector<vector<int>>tree(2, vector<int>(4*(n+1)));
    vector<vector<int>>lazy(2, vector<int>(4*(n+1)));
    ...
    dfs(1, 0, graph, visit, zero_one, in, out, v, arr);
    build(arr[0], tree[0], 1, 1, n);
    build(arr[1], tree[1], 1, 1, n);
    
    while(m--){
        int a;  cin>>a;
        if(a==1){
            int x,val;  cin>>x>>val;
            lazy_update(tree[zero_one[x]]  , lazy[zero_one[x]]  , 1, 1, n, in[x], out[x], val);
            lazy_update(tree[zero_one[x]^1], lazy[zero_one[x]^1], 1, 1, n, in[x], out[x], -val);
        }
        else{
            int x;  cin>>x;
            cout<<lazy_query(tree[zero_one[x]], lazy[zero_one[x]], 1, 1, n, in[x], in[x])<<endl;
        }
    }
return;
}
