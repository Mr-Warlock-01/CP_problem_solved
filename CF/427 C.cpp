...
int lowest_cost_in_cycle = LLMax;
int lowest_ways_in_cycle=1;


void in_stack(int x, vector<int>graph[],  vector<bool>&visit, stack<int>&st){
    visit[x]=1;
    for(int i=0; i<graph[x].size(); i++){
        if(!visit[graph[x][i]]){
            in_stack(graph[x][i], graph, visit, st);
        }
    }
    st.push(x);
}
...
void ssc(int x, vector<int>graph[],  vector<bool>&visit, vector<int>&cost){
    visit[x]=1;
    if(lowest_cost_in_cycle>cost[x]){
        lowest_cost_in_cycle=cost[x];
        lowest_ways_in_cycle=1;
    }
    else if(lowest_cost_in_cycle==cost[x]){
        lowest_ways_in_cycle++;
    }

    for(int i=0; i<graph[x].size(); i++){
        if(!visit[graph[x][i]]){
            ssc(graph[x][i], graph, visit, cost);
        }
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>rev_graph[n+1];
    vector<bool>visit(n+1), rev_visit(n+1);
    stack<int>st;
    ...
    for(int i=1; i<=n; i++){
        if(!visit[i]){
            in_stack(i, graph, visit, st);
        }
    }

    while(!st.empty()){
        if(!rev_visit[st.top()]){
                lowest_cost_in_cycle = LLMax;
                lowest_ways_in_cycle=1;
            ssc(st.top(), rev_graph, rev_visit, cost);
                low_cost+=lowest_cost_in_cycle;
                ways*=lowest_ways_in_cycle;
                ways%=MOD;
        }
        st.pop();
    }
