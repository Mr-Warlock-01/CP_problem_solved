...
const int N= 500001;    
int child_count[N]; //child_count[0]=-1;
int level_reach[N]; //level_reach[0]=-1;
int level[N];       //level[0]=-1;

int n;

int dfs1(int x, int par, int lev, vector<int>graph[]){
    int ans=0;  level[x]=lev;
    for(int i=0; i<graph[x].size(); i++){
        if(graph[x][i]!=par){
            ans+=(dfs1(graph[x][i], x, lev+1, graph)+1);
        }
    }
    return child_count[x]=ans;
}

int dfs2(int x, int par, int lev, vector<int>graph[]){
    int ans=lev;
    for(int i=0; i<graph[x].size(); i++){
        if(graph[x][i]!=par){
            ans=max(ans, dfs2(graph[x][i], x, lev+1, graph));
        }
    }
    return level_reach[x]=ans;
}
...
int bs(int k){
    ...
    while(l<=r){
        int mid=l+((r-l)/2);
        if(level_reach[mid]<k){ans=mid; l=mid+1;}
        else{r=mid-1;}
    }
    return ans;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>node_cnt_by_level(n+1);
    for(int i=1; i<n; i++){
        int u,v;    cin>>u>>v;
        graph[u].pb(v);
        graph[v].pb(u);
    ...
    dfs1(1, -1, 0, graph);
    dfs2(1 ,-1, 0, graph);
    sort(level_reach+1 , level_reach+n+1);

    for(int i=1; i<=n; i++){node_cnt_by_level[level[i]]+=child_count[i];}
    
    int k=0;
    for(int i=1; i<=n; i++){k=max(k, level[i]);}
    int ans=n-1;
    while(k){
        int ans_check=node_cnt_by_level[k];
        ans_check+=bs(k);
        db(k);  db(ans_check)
