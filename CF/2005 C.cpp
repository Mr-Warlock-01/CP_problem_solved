...
int xxx(int pos,int flag, string s[], vector<vector<int>>&dp){
    if(pos==n){return 0;}
    if(dp[pos][flag]!=-1){return dp[pos][flag];}

    int a=xxx(pos+1, flag, s, dp);

    int ok=0;
    int x=flag;
    for(int i=0; i<m; i++){
             if((s[pos][i]=='n') && (x==0)){x=1;}
        else if((s[pos][i]=='a') && (x==1)){x=2;}
        else if((s[pos][i]=='r') && (x==2)){x=3;}
        else if((s[pos][i]=='e') && (x==3)){x=4;}
        else if((s[pos][i]=='k') && (x==4)){x=0; ok+=10;}
        
             if(s[pos][i]=='n'){ok--;}
        else if(s[pos][i]=='a'){ok--;}
        else if(s[pos][i]=='r'){ok--;}
        else if(s[pos][i]=='e'){ok--;}
        else if(s[pos][i]=='k'){ok--;}
    }
    int b=xxx(pos+1, x, s, dp)+ok;
    return dp[pos][flag]=max(a,b);
}
