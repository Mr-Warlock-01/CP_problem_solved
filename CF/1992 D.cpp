...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    int on_land=1;
    while(1){
        if((on_land==1) && ((a+ju)>=n)){ans=1; break;}
        if(on_land==1){
            bool land=0;
            int water=-1;
            for(int i=1; i<=ju; i++){
                if((a+1)==n){ans=1; break;}
                if(s[a+i]=='L'){a+=i; land=1; break;}
                if(s[a+i]=='W'){water=a+i;}
            }
            if(land==1){continue;}
            else if(water!=-1){a=water; on_land=0;}
            else{ans=0; break;}
        }
        else{
            sw--;
            a++;
            if(a==n){ans=1; break;}
            if(s[a]=='C'){ans=0; break;}
            else if(s[a]=='W'){continue;}
            else{on_land=1;}
