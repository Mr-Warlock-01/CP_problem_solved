...
ll magic_0_to_n(ll n, ll k, ll pow){
    if(n<k){return 0;}
    return ((n-k)/pow)+1;
}

void solve(int T){            ///   (DRINK WATER)
    ll l,r,p,k;    cin>>l>>r>>p>>k;
    ll ans=xor_l_to_r(l,r);

    ll mg_r=magic_0_to_n(r,k,powMod(2,p));
    ll mg_l=magic_0_to_n(l-1,k,powMod(2,p));

    if(mg_r!=0){
        bool flag=0;
        if(mg_r%2==1 && k!=0){flag=1;}
        if(mg_r%2==0 && k==0){flag=1;}
        mg_r=xor_1_to_n(mg_r-1);
        mg_r<<=p;
        if(flag){mg_r^=k;}
    }
    ...
    if(mg_l!=0){
        bool flag=0;
        if(mg_l%2==1 && k!=0){flag=1;}
        if(mg_l%2==0 && k==0){flag=1;}
        mg_l=xor_1_to_n(mg_l-1);
        mg_l<<=p;
        if(flag){mg_l^=k;}
    }
