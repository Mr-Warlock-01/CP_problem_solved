...
pair<int,int>arr[N];
int y_axis[N];
gp_hash_table<int, int,custom_hash>mp;//y_axis;
...
int xxx(int i, int val, wavelet_tree &Tr){
    int l=1;
    int r=n;
    while(l<=r){
        int miid=(l+r)/2;
        int l_d=Tr.LTE(1,i-1,miid-1);
        int r_d=Tr.LTE(i,n,miid-1);
        int l_u=i-1-l_d;
        int r_u=n-(i-1+r_d);
        bool flag=0;
        if(l_d<val || r_d<val){l=miid+1; flag=1;}
        if(l_u<val || r_u<val){r=miid-1; flag=1;}
        if(!flag){return miid;}
    }
    return -1;
}
...
void solve(int T){            ///   (DRINK WATER)
    ...
    for(int i=1; i<=n; i++){
        y_axis[i]=mp[arr[i].ss];
    }

    wavelet_tree Tr(y_axis+1,y_axis+n+1,0,n+1);

    int ans=0;
    int ans_x=0, ans_y=0;
    int z=arr[1].ff;;
    for(int i=1; i<=n; i++){
        if(z==arr[i].ff){continue;}
        z=arr[i].ff;
        int l=ans+1;
        int r=n/4;
        while(l<=r){
            int mid=(l+r)/2;
            int flag=xxx(i, mid, Tr);
            if(flag==-1){r=mid-1;}
            else{
                ans=mid;
                ans_x=i;
    ...
    if(ans==0){cout<<"0"<<endl<<"0 0"<<endl;;}
    else{
        for(int i=1; i<=n; i++){
            if(mp[arr[i].ss]==ans_y){ans_y=arr[i].ss; break;}
        }
        cout<<ans<<endl;
        cout<<arr[ans_x].ff<<" "<<ans_y<<endl;
    }
return;
}
