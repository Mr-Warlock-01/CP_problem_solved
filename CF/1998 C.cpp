...
int BS(vector<pair<int,bool>>&v){
    ...
    while(l<=r){
        ...
        for(int i=n-1; i>=0; i--){
            if(b==z){break;}
            if(v[i].ff<mid && v[i].ss==0){a++;}
            else{
                b++;
                if(v[i].ff<mid && v[i].ss==1){ok+=(mid-v[i].ff);}
            }
            if(b==z){break;}
        }
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    if(x==-1){cout<<v[n-1].ff+v[median_1].ff<<endl; return;}
    if(median_1<x){ans=max(ans, v[x].ff+v[median_1].ff+k);}
    else{ans=max(ans, v[x].ff+v[median_2].ff+k);}
    ...
    for(int i=n-1; i>=0; i--){
        if(!cnt){break;}
        if(v[i].ff>=z){cnt--;}
        else if(v[i].ff<z && v[i].ss==1){k-=(z-v[i].ff); v[i].ff=z; cnt--;}
    }

    sort(all(v));
    ans=max(ans, v[n-1].ff+v[median_1].ff);
    if(v[median_1].ss){
        ans=max(ans, v[n-1].ff+ min(v[median_2].ff,v[median_1].ff+k));
    }

    for(int i=n-1; i>=0; i--){if(v[i].ss){x=i; break;}}
    if(median_1<x){ans=max(ans, v[x].ff+v[median_1].ff+k);}
    else{ans=max(ans, v[x].ff+v[median_2].ff+k);}
