    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            if(v[i][j]=='.'){ans[i][j]='.';}
                    ...
                    int lol=1;
                    if((i>0) && (v[i-1][j]=='.')){
                        lol+=node_reach[i-1][j].ff;
                        flag[0]=node_reach[i-1][j].ss;
                    }
                    if(((i+1)<n) && (v[i+1][j]=='.') && (flag[0]!=node_reach[i+1][j].ss)){
                        lol+=node_reach[i+1][j].ff;
                        flag[1]=node_reach[i+1][j].ss;
                    }
                    if((j>0) && (v[i][j-1]=='.') && (flag[0]!=node_reach[i][j-1].ss) && (flag[1]!=node_reach[i][j-1].ss)){
                        lol+=node_reach[i][j-1].ff;
                        flag[2]=node_reach[i][j-1].ss;
                    }
                    if(((j+1)<m) && (v[i][j+1]=='.') && (flag[0]!=node_reach[i][j+1].ss) && (flag[1]!=node_reach[i][j+1].ss) && (flag[2]!=node_reach[i][j+1].ss)){
                        lol+=node_reach[i][j+1].ff;
                    }
                    lol%=10;
                    ans[i][j]=(char)(lol+48);
