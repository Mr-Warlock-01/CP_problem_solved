...
void xxx(){
    int x=1;
    for(int i=0;i <62; i++){
        v[i].ff=x;  x*=2;   v[i].ss=x;
    }
return;
}

int ans(int x, string s){
    int level;
    bool flag=0;   //0=left, 1=right;
    for(int i=0; i<62; i++){
        if((x-v[i].ff)%v[i].ss==0){level=i; break;}
    }
    int zzz=(x-v[level].ff)/v[level].ss;
    if(zzz%2){flag=1;}

    for(int i=0; i<s.size(); i++){
        if(s[i]=='L' && level>0){      level--;  flag=0;  x-=v[level].ff;}
        else if(s[i]=='R' && level>0){ level--;  flag=1;  x+=v[level].ff;}
        else if(s[i]=='U' && level<haha){
            if(flag==0){x+=v[level].ff;}
            else{x-=v[level].ff;}
            level++;
            zzz=(x-v[level].ff)/v[level].ss;
            if(zzz%2){flag=1;}
            else{flag=0;}
        }
    }
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>z(q);
    vector<string>s(q);
    for(int i=0; i<q; i++){cin>>z[i]>>s[i];}

    for(int i=0; i<62; i++){if(v[i].ss>n){haha=i; break;}}

    for(int i=0; i<q; i++){cout<<ans(z[i], s[i])<<endl;}
}
