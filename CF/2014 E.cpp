...
void bfs(int node,  vector<vector<ll>>&time, vector<pair<int,ll>>graph[], vector<bool>&horse){
    priority_queue<tuple<bool,ll,int>, vector<tuple<bool,ll,int>>, greater<tuple<bool, ll,int>>>pq;
    pq.push({0,0,node});
    time[0][node]=0;

    while(!pq.empty()){
        bool flag=get<0>(pq.top());
        ll w   = get<1>(pq.top());
        int x   = get<2>(pq.top());
        pq.pop();
        if(horse[x]){flag=1;}
        if(time[flag][x]>=w){
            for(int i=0; i<graph[x].size(); i++){
                ...
                ll www=graph[x][i].ss;
                if(flag){www/=2;}
                if((w+www)<time[flag][node]){
                    time[flag][node]=w+www;
                    pq.push({flag,w+www, node});
                }   
            }
        }
    }
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<vector<ll>>time_1(2,vector<ll>(n+1,LLMax));
    vector<vector<ll>>time_2(2,vector<ll>(n+1,LLMax));
    ...
    bfs(1, time_1, graph, horse);
    bfs(n, time_2, graph, horse);
    
    ll ans=LLMax;
    for(int i=1; i<=n; i++){
        ans=min(ans, max(min(time_1[0][i], time_1[1][i]), min(time_2[0][i], time_2[1][i])));
    }
