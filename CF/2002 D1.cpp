...
void dfs(int p, int z, vector<int>&parent_index_no, vector<pair<int,int>>&child_index_no){
    int ok=ind;   ind++;
    parent_index_no[ok]=p;
    if(z==0){return;}
    child_index_no[ok].ff=ind;
    dfs(ok, z-1, parent_index_no, child_index_no);
    child_index_no[ok].ss=ind;
    dfs(ok, z-1, parent_index_no, child_index_no);
return;
}


bool child_check(int x, int a, int b, vector<pair<int,int>>&child, vector<int>&v){
    if((child[x].ff==-1 && child[x].ss==-1) && (a==-1 && b==-1)){return 1;}
    else if(child[x].ff==-1 && child[x].ss==-1){return 0;}
    else if(a==-1 && b==-1){return 0;}
    else if(child[x].ff==v[a] && child[x].ss==v[b]){return 1;}
    else if(child[x].ff==v[b] && child[x].ss==v[a]){return 1;}
return 0;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>v(n+1);
    vector<int>parent_index_no(n+1,-1);
    vector<pair<int,int>>child_index_no(n+1,{-1,-1});

    for(int i=2; i<=n; i++){
        int a;  cin>>a;
        parent[i]=a;
        if(i%2==0){child[a].ff=i;}
        else{child[a].ss=i;}
    }
    ...
    int z=log2(n);  ind=1;
    dfs(0, z, parent_index_no, child_index_no);
    parent_index_no[1]=0;
    child_index_no[0]=child[0]={1,1};

    vector<bool>ans(n+1);
    int false_count=0;    

    for(int i=0; i<=n; i++){
        if(child_check(v[i], child_index_no[i].ff, child_index_no[i].ss, child, v)){ans[i]=1;}
        else{false_count++;}
    }
    ...
    while(q--){
        ...
        int flag=child_check(v[x], child_index_no[x].ff, child_index_no[x].ss, child, v);
        if(flag!=ans[x]){
            if(flag==1){ans[x]=1;  false_count--;}
            else{       ans[x]=0;  false_count++;}
        }

        flag=child_check(v[y], child_index_no[y].ff, child_index_no[y].ss, child, v);
        if(flag!=ans[y]){
            if(flag==1){ans[y]=1;  false_count--;}
            else{       ans[y]=0;  false_count++;}
        }

        /////////////parents case handle
        int xx=parent_index_no[x];
        flag=child_check(v[xx], child_index_no[xx].ff, child_index_no[xx].ss, child, v);
        if(flag!=ans[xx]){
            if(flag==1){ans[xx]=1;  false_count--;}
            else{       ans[xx]=0;  false_count++;}
        }

        int yy=parent_index_no[y];
        flag=child_check(v[yy], child_index_no[yy].ff, child_index_no[yy].ss, child, v);
        if(flag!=ans[yy]){
            if(flag==1){ans[yy]=1;  false_count--;}
            else{       ans[yy]=0;  false_count++;}
        }
        ////////////////////////////
        if(false_count==0){YES;}
        else{NO;}
    }
return;
}
