...
int prefixSum(int i, vector<int>&pre_sum){
    if(i<0){return 0;}
    return pre_sum[i];
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    while(q--){
        ...
        int RR=r/n;
        int r_sum=RR*pre_sum[n-1];
        if((r%n)!=0){
            int a=RR-1;
            int b=(RR+r-1)%n;
            if(a>=0){r_sum-=pre_sum[a];}
            r_sum+=pre_sum[b];
            if(a>b){r_sum+=pre_sum[n-1];}
        }

        int LL=l/n;
        int l_sum=LL*pre_sum[n-1];
        if((l%n)!=0){
            int a=LL-1;
            int b=(LL+l-1)%n;
            if(a>=0){l_sum-=pre_sum[a];}
            l_sum+=pre_sum[b];
            if(a>b){l_sum+=pre_sum[n-1];}
        }
