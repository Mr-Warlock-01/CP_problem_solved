...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    int x=sqrt(cnt);
    if((x*x)!=cnt){cout<<"TRIANGLE"<<endl; return;}
    int xxx,yyy;
    for(int i=0; i<n; i++){
        bool h=1;
        for(int j=0; j<n; j++){
            if(arr[i][j]=='1'){xxx=i; yyy=j;  h=0; break;  }
        }
        if(h==0){break;}
    }
    int abc=0;
    for(int i=xxx; i<(xxx+x) && i<n; i++){
        for(int j=yyy; j<(yyy+x) && j<n; j++){
            if(arr[i][j]=='1'){abc++;}
        }
    }
    if(abc==cnt){cout<<"SQUARE"<<endl;}
