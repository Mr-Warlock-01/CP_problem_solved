...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    cin>>v;
    for(int i=0; i<n; i++){for(int j=0; j<m; j++){char c; cin>>c;if(c=='1'){check[i][j]=1;}}}

    int sum=0;
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            if(check[i][j]){sum+=v[i][j];}
            else{sum-=v[i][j];}
        }
    }

    if(sum==0 || k==1){YES; return;}
    vector<vector<int>>arr(n, vector<int>(m+1-k));
    //int arr[n][m+1-k];
