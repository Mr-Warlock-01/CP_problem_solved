...
void putting_in_stack(int x){
    vis[x]=1;
    if(vis[graph[x]]==0){
        putting_in_stack(graph[x]);
    }
    st.push(x);
return;
}
...
void scc(int x, int p){
    par[x]=p;
    for(int i=0; i<rev_graph[x].size(); i++){
        if(par[rev_graph[x][i]]==0){
            scc(rev_graph[x][i], p);
        }
    }
return;
...
void solve(int T){            ///   (DRINK WATER) 
    ...
    for(int i=1; i<=n; i++){rev_graph[graph[i]].pb(i);}

    for(int i=1; i<=n; i++){
        if(!vis[i]){putting_in_stack(i);}
    }
    ...
    for(int i=1; i<=n; i++){
        int node=par[i];
        for(auto u:rev_graph[i]){
            if(par[u]!=node){
                new_graph[node].insert(par[u]);
            }
        }
    }

    //ans calculation clean
    for(int i=1; i<=n; i++){
        vis[i]=0;
        ans[i]=-1; sz[i]=-1;
    }
