...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n;  cin>>n;
    vector<char>v(n),z(9);   cin>>v;   cin>>z;

    bool flag=0;
    int l=-1,r=-1;

    for(int i=0; i<n; i++){
        if(flag==0 && v[i]<z[v[i]-'1']){l=i; r=n; flag=1;}
        else if(flag==1 && v[i]>z[v[i]-'1']){r=i; break;}
    }

    if(!flag){for(int i=0; i<n; i++){cout<<v[i];}}
    else{
        for(int i=0; i<l; i++){cout<<v[i];}
        for(int i=l; i<r; i++){cout<<z[v[i]-'1'];}
