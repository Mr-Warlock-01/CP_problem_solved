...
ll dfs(int x, int par, bool flag){   //flag=0=low, flag=1=high
    if(dp[x][flag]!=-1){return dp[x][flag];}
    ll ans=0;
    for(int u:graph[x]){
        if(par!=u){
            ll a=dfs(u,x,flag);
            if(flag==0){a+=abs(cost[x].ff-cost[u].ff);}
            else{a+=abs(cost[x].ss-cost[u].ss);}
            ll b=dfs(u,x,flag^1);
            if(flag==0){b+=abs(cost[x].ff-cost[u].ss);}
            else{b+=abs(cost[x].ss-cost[u].ff);}
            ans+=max(a,b);
        }
    }
...
void solve(int T){            ///   (DRINK WATER)
    ...
    ll ans=max(dfs(1,-1, 0),dfs(1,-1, 1));
    cout<<ans<<endl;
return;
}
