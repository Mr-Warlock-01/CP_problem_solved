...
bool validity_check(vector<int>&v){
    int sz=v.size();
    int mx=-1;
    for(int i=0; i<sz; i++){
        if(v[i]==sz){mx=i; break;}
    }
    if(mx==-1){return 0;}
...
void solve(int T){            ///   (DRINK WATER) 
    ...
    if(flag){for(int i=1; i<=n; i++){cout<<i<<" ";} return;}
    ...
    if(flag){for(int i=n; i>0; i--){cout<<i<<" ";} return;}
    ...
    while(l<=r){
        if(dq.size()==0){break;}
        pair<int,int>left=dq.back();
        pair<int,int>right=dq.front();
        if(vis[val]){
            if(dq.size()==1){break;}
            if(left.ff==val){ 
                if(v[l]==val){val++; l++; dq.pop_back();}
                else{flag=0; break;  }
            }
            else if(right.ff==val){
                if(v[r]==val){val++; r--; dq.pop_front();}
                else{flag=0; break;}
            }
            else{flag=0; break;}
            continue;
        ...
        if(left.ff==right.ff){
            ...
            if(b==0){b+=1000000;}
            if(a<b){
                if(v[l]==0){v[l]=val; vis[val]=1; l++; val++;}
                else if(v[r]==0){v[r]=val; vis[val]=1; r--; val++;}
                else if(v[l]==left.ff){dq.pop_back(); l++;}
            }
            else{
                if(v[r]==0){v[r]=val; vis[val]=1; r--; val++;}
                else if(v[l]==0){v[l]=val; vis[val]=1; l++; val++;}
                else if(v[r]==right.ff){dq.pop_front(); r--;}
            }
        }
    ...
    if(flag){
        val++;
        for(int i=0; i<n; i++){
            if(v[i]==0){v[i]=val; val++;}
        }
        flag=validity_check(v);
    }
    if(flag){cout<<v<<endl;}
    else{cout<<"*"<<endl;}
return;
