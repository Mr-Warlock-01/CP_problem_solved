...
pair<pair<int,int>,pair<int,int>>edge[N];   //(cost,idx)(u,v)
set<pair<int,int>>graph[N];  //u,idx
int parent[N];
bool vis[N];
int in[N];
int low[N];
int ans[N]; //0=none, 1= at least one, 2=any;
int timer=1;
...
void bridge_find(int x, int p){
    vis[x]=1;
    in[x]=low[x]=(timer++);
    for(auto [u,idx]:graph[x]){
        if(u==p){continue;}
        else if(vis[u]){
            low[x]=min(low[x], in[u]);
        }
        else{
            bridge_find(u, x);
    ...
    ///////
    pair<int,int>check={0,0};   // node, pos
    for(auto [u,idx]:graph[x]){
        if(u==check.ff){
            ans[check.ss]=ans[idx]=1;
        }
        check={u,idx};
    }
}
...
void solve(int T){
    int n,m;    cin>>n>>m;
    for(int i=0; i<m ;i++){
        cin>>edge[i].ss.ff>>edge[i].ss.ss>>edge[i].ff.ff;
        edge[i].ff.ss=i;
    }
    edge[m].ff.ff=1e9;
    ...
    int l=0,r=0; 
    stack<pair<pair<int,int>,int>>st;   //(u,v),idx
    while(r<=m){
        if(edge[l].ff.ff!=edge[r].ff.ff){
            int old_l=l;
            set<int>ss;
            while(l<r){
                int u= edge[l].ss.ff;
                int v= edge[l].ss.ss;
                int pos=edge[l].ff.ss;
                u=find_set(u);
                v=find_set(v);
                if(u!=v){
                    ans[pos]=1;
                    graph[u].insert({v,pos});
                    graph[v].insert({u,pos});
                    ss.insert(u);
                    ss.insert(v);
                }
                l++;
            ...
            while(old_l<r){
                int u= edge[old_l].ss.ff;
                int v= edge[old_l].ss.ss;
                int pos=edge[old_l].ff.ss;
