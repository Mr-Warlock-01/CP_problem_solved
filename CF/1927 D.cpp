...
int BS(int arr[],int n, int x){
    ...
    while(l<=r){
        ...
        else if(arr[mid]<x){ans=max(ans,mid); l=mid+1;}
        else{r=mid-1;}
    }
    return ans;
}
...
void pikachu(){         //MAIN
    ...
    int q;  cin>>q;
    vector<pair<int,int>>que(q);     for(int i=0; i<q ;i++){cin>>que[i].first>>que[i].second;}

    int lol=-1;
    int track[n];
    int sz=0;
    for(int i=0; i<n; i++){
        if(lol!=v[i]){track[sz]=i;    lol=v[i];   sz++;}
    }

    for(int i=0; i<q; i++){
        int l=BS(track, sz, que[i].first-1);
        int r=BS(track, sz, que[i].second-1);
        if(l==r){cout<<"-1 -1"<<endl;}
        else{
            cout<<   (max(track[l],que[i].first-1)+1)  <<" "<<    (track[l+1]+1)   <<endl;
        }
    }
    cout<<endl;
}
