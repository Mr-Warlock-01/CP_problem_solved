...
void solve(int T){            ///   (DRINK WATER)
    ...
    for(int i=0; i<n; i++){
        while(!pq.empty()){
            if(pq.top()<v[i].ff){pq.pop();}
            else{break;}
        }
        if(last_val!=v[i].ff){
            ll z=(n-i+pq.size())*v[i].ff;
            if(pq.size()<=k){ans=max(ans,z);}
        }
        last_val=v[i].ff;
        pq.push(v[i].ss);
    }
    ...
    for(int i=0; i<n; i++){
        for(int j=idx; j<n; j++){
            if(v[j].ff<b[i]){idx++; pq.push(v[j].ss);}
            else{break;}
        }
        while(!pq.empty()){
            if(pq.top()<b[i]){pq.pop();}
            else{break;}
        }
        if(last_val!=b[i]){
            int sz=pq.size();
            int aa=b[i];
            ll z=(n-idx+pq.size())*b[i];
            if(pq.size()<=k){ans=max(ans,z);}
        }
        last_val=b[i];
