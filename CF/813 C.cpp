...
void dfs(int x, vector<int>graph[], vector<int>&visit, int point){
    visit[x]=point;
    for(int z:graph[x]){
        if(visit[z]==-1){dfs(z, graph, visit, point+1);}
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>visit_1(n+1, -1), visit_x(n+1, -1);
    for(int i=1; i<n; i++){
        int u,v;    cin>>u>>v;
        graph[u].pb(v);     graph[v].pb(u);
    }

    dfs(1, graph, visit_1, 0);
    dfs(x, graph, visit_x, 0);

    int ans=0;
    for(int i=0; i<=n; i++){
        if(visit_1[i]>visit_x[i]){ans=max(ans, visit_1[i]);}
    }
    ans*=2;
    cout<<ans;
}
