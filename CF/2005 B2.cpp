...
xxx_1(int value, vector<int>&v){
    int ans=-1;
    int l=0, r=m-1;
    while(l<=r){
        int mid=(l+r)/2;
        if(v[mid]<value){ans=v[mid];   l=mid+1;}
        else{r=mid-1;}
    }
    return ans;
}

xxx_2(int value, vector<int>&v){
    int ans=-1;
    int l=0, r=m-1;
    while(l<=r){
        int mid=(l+r)/2;
        if(v[mid]>value){ans=v[mid];   r=mid-1;}
        else{l=mid+1;}
    }
    return ans;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int q;  cin>>n>>m>>q;
    vector<int>v(m);    cin>>v; sort(all(v));
