...
bool BS(vector<int>&damage, vector<double>&cooldown, int x, int h, int n){
    int sum=0;
    for(int i=0; i<n; i++){
        sum+=(damage[i]*(floor(x/cooldown[i])+1));
    }
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int h,n;    cin>>h>>n;
    vector<int>damage(n);   cin>>damage;
    vector<double>cooldown(n); cin>>cooldown;
    ...
    while(l<=r){
        int mid=(l+r)/2;
        bool flag=BS(damage, cooldown, mid, h, n);
