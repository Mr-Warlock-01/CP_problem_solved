...
int dfs(int x, vector<pair<int,bool>>graph[], bool vis[]){
    vis[x]=1;
    int ans=1;
    for(int i=0; i<graph[x].size(); i++){
        if(vis[graph[x][i].ff]==0 && graph[x][i].ss==0){
            ans+=dfs(graph[x][i].ff, graph, vis);
        }
    }
return ans;
}
...
void solve(int T){            ///   (DRINK WATER)
    int n,k;    cin>>n>>k;
    vector<pair<int,bool>>graph[n+1];
    ...
    ll ans=powMod(n,k,MOD);

    for(int i=1; i<=n; i++){
        if(!vis[i]){
            int z=dfs(i,graph,vis);
            ans=(ans - powMod(z,k,MOD)+ MOD) % MOD;
        }
    }
    cout<<ans<<endl;
return;
