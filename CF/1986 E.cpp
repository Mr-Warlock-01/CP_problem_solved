...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<n; i++){ok.pb({v[i]%k,v[i]});}
    sort(all(ok));
    ...
    for(int i=0; i<n; i++){
        ...
        xxx[mp[ok[i].ff]].pb(ok[i].ss);
    }

    int ans=0;

    for(int i=1; i<haha; i++){
        if(xxx[i].size()%2==1 && flag==0){cout<<"-1"<<endl; return;}
        if(xxx[i].size()%2==0){
            for(int j=1; j<xxx[i].size(); j+=2){
                ans+=xxx[i][j]-xxx[i][j-1];
            }
        }
        else{
            flag=0;
            if(xxx[i].size()==1){continue;}
            vector<int>a,b;
            a.pb(0);
            for(int j=1; j<xxx[i].size(); j++){
                if(j%2==1){a.pb(xxx[i][j]-xxx[i][j-1]);}
                else{b.pb(xxx[i][j]-xxx[i][j-1]);}
            }
            b.pb(0);
            int ans_ok=LLMax;
            for(int j=1; j<a.size(); j++){a[j]+=a[j-1];}
            for(int j=b.size()-2; j>=0; j--){b[j]+=b[j+1];}
            for(int j=0; j<a.size(); j++){ans_ok=min(ans_ok, a[j]+b[j]);}
            ans+=ans_ok;
        }
    }
    cout<<ans/k<<endl;
