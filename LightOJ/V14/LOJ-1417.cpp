...
void dfs_putting_in_stack(int x, vector<int>graph[], vector<bool>&visit, stack<int>&st){
    visit[x]=1;
    for(int i=0; i<graph[x].size(); i++){
        if(!visit[graph[x][i]]){
            dfs_putting_in_stack(graph[x][i], graph, visit, st);
        }
    }
    st.push(x);
return;
...
void scc_dfs(int x, vector<int>scc_graph[], vector<bool>&visit , priority_queue<int,vector<int>, greater<int>>&pq, set<int>&sss){
    visit[x]=1;     pq.push(x);     sss.insert(x);
    for(int i=0; i<scc_graph[x].size(); i++){
        if(!visit[scc_graph[x][i]]){
            scc_dfs(scc_graph[x][i], scc_graph, visit, pq, sss);
        }
    }
}


void dfs(int x, vector<int>graph[], vector<int>&weight, vector<bool>&visit, vector<int>&denote, vector<int>&ans){
    visit[x]=1; ans[x]=weight[x];
    for(int i=0; i<graph[x].size(); i++){
        if(!visit[denote[graph[x][i]]]){
            dfs(denote[graph[x][i]], graph, weight, visit, denote, ans);
        }
        ans[x]=max(ans[x], weight[x]+ans[denote[graph[x][i]]]);
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n;  cin>>n;
    vector<int>graph[n+1] , scc_graph[n+1];
    vector<bool>visit_1(n+1), visit_2(n+1), visit_3(n+1);
    stack<int>st;
    vector<int>weight(n+1, 1);
    vector<int>denote(n+1);             for(int i=0; i<=n; i++){denote[i]=i;}
    vector<int>ans(n+1);
    ...
    for(int i=1; i<=n; i++){
        if(!visit_1[i]){
            dfs_putting_in_stack(i, graph, visit_1, st);
        }
    ...
    while(!st.empty()){
        ...
        if(!visit_2[x]){
            ...
            set<int>sss;
            scc_dfs(x, scc_graph, visit_2, pq, sss);
            if(pq.size()!=1){
                int denoting_node=pq.top();
                weight[denoting_node]=pq.size();
                ordered_set<int>os;
                while(!pq.empty()){
                    for(int i=0; i<graph[pq.top()].size(); i++){
                        if(*sss.find(graph[pq.top()][i])!=graph[pq.top()][i]){
                            os.insert(graph[pq.top()][i]);
                        }
                    }
                    graph[pq.top()].clear();
                    denote[pq.top()]=denoting_node;
                    pq.pop();
                }
                for(int i=0; i<os.size(); i++){
                    graph[denoting_node].pb(*os.find_by_order(i));
                }
            }
        }
    }
    ...
    for(int i=1; i<=n; i++){
        if(!visit_3[denote[i]]){
            dfs(denote[i], graph, weight, visit_3, denote, ans);
        }
    }
    ...
    int flag=0;
    for(int i=1; i<=n; i++){
        if(ans[i]>flag){flag=ans[i];    answer=i;}
    }
