...
const int N = 26; // change here
const char base_char = 'a';  // change here
...
void query(const string &s, int start) { // "s" is a prefix of some element or not
    int n=s.size();
    TrieNode *cur = root;
    for(int i=start; i<n; i++) {
        int idx = s[i] - base_char;
        if (cur -> nxt[idx] == NULL){return;}
        cur = cur -> nxt[idx];
        for(int j=0; j<cur->cnt.size();j++){
            ans[cur->cnt[j]]++;
        }
    }
return;
