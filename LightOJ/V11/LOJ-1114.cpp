...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    root = new TrieNode();
    for(int i=0; i<n; i++){
        if(v[i].size()>2){sort(v[i].begin()+1, v[i].begin()+v[i].size()-1);}
        insert(v[i]);
    }
    ...
    for(int i=0; i<q; i++){
        ...
        for(int i=0; i<s.size(); i++){
            if(s[i]!=' '){zzz.pb(s[i]);}
                ...
                if(zzz.size()>2){sort(zzz.begin()+1, zzz.begin()+zzz.size()-1);}
                if(zzz=="" || zzz==" "){zzz="";continue;}
                ans*=query(zzz);
                zzz="";
            }
        }
