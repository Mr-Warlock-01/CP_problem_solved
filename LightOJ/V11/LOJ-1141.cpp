...
vector<bool>v(1000,1);
vector<int>divisor[1000];       //prime divisor
...
void bfs(int a, int b, int &ans){
    ...
    while(!qq.empty()){
        ...
        for(int i=0; i<divisor[x].size(); i++){
            if(((x+divisor[x][i])<b) && (os.order_of_key(x+divisor[x][i]) == os.order_of_key(x+divisor[x][i]+1))){
                qq.push({x+divisor[x][i], d+1});
                os.insert(x+divisor[x][i]);
            }
            else if((x+divisor[x][i])==b){ans=d+1; return;}
        }
    }
return;
}
