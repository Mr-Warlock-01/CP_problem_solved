...
int xxx(int p, int q, int r, vector<vector<vector<int>>>&dp){
    if(p==x){return 0;}
    if(q==y){return 0;}
    if(r==z){return 0;}
    ...
    if((a[p]==b[q]) && (b[q]==c[r])){
        return dp[p][q][r]=xxx(p+1,q+1,r+1,dp)+1;
    }
    return dp[p][q][r]=max({xxx(p+1,q,r,dp),xxx(p,q+1,r,dp),xxx(p,q,r+1,dp)});
}
