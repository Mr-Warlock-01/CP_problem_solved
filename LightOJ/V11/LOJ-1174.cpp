...
void bfs(int x, vector<int>graph[], vector<int>&dis, int n){
    vector<int>visit(n);
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    int s,d;    cin>>s>>d;
    vector<int>dis_from_s(n), dis_to_d(n);

    bfs(s, graph, dis_from_s, n);
    bfs(d, graph, dis_to_d, n);

    int ans=0;
    for(int i=0; i<n; i++){
        ans=max(ans, dis_from_s[i]+dis_to_d[i]);
    }
