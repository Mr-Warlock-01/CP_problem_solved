...
void dsu(int x, int p, vector<pair<int,int>>graph[], vector<pair<int,int>>&parent_lca, vector<int>&level, int l){
    for(int i=0; i< graph[x].size(); i++){
        if(p!=graph[x][i].ff){
            parent_lca[graph[x][i].ff].ff=x;
            parent_lca[graph[x][i].ff].ss=graph[x][i].ss;
            level[graph[x][i].ff]=l+1;
            dsu(graph[x][i].ff, x, graph, parent_lca, level, l+1);
        }
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>parent_dsu(n+1), dsu_size(n+1);
    vector<pair<int,int>>parent_lca(n+1);       //parent,weight
    ...
    for(int  i=0; i<m; i++){
        if(x){
            pq.push({edge[i].ff, edge[i].ss.ff, edge[i].ss.ss});
            int a= find_set(edge[i].ss.ff, parent_dsu, dsu_size);
            int b= find_set(edge[i].ss.ss, parent_dsu, dsu_size);
            if(a!=b){
                x--;
                union_sets(edge[i].ss.ff, edge[i].ss.ss, parent_dsu, dsu_size);
            }
        }
        /////////////
        if(x==0 && spaning_tree==0){
            spaning_tree=1;
            for(int i=0; i<=n; i++){
                parent_dsu[i]=i;
                dsu_size[i]=1;
            ...
            while(!pq.empty()){
                ...
                int v=get<2>(pq.top());
                pq.pop();
                if(find_set(u, parent_dsu, dsu_size)!= find_set(v, parent_dsu, dsu_size)){
                    ans+=w;
                    graph[u].pb({v,w});         graph[v].pb({u,w});
                    union_sets(u, v, parent_dsu, dsu_size);
                }
            }
        }
        else if(spaning_tree==1){
            dsu(1,-1, graph, parent_lca, level, 0);
            int a= edge[i].ss.ff;
            int b= edge[i].ss.ss;
            int w=edge[i].ff;
            int u=a, v=b;
            ...
            while(d>0){
                if(www<parent_lca[b].ss){
                    www=parent_lca[b].ss;
                    AA=b;   BB=parent_lca[b].ff;
                }
                b=parent_lca[b].ff;
                d--;
            }
            while(a!=b){
                if(www<parent_lca[a].ss){
                    www=parent_lca[a].ss;
                    AA=a;   BB=parent_lca[a].ff;
                }
                a=parent_lca[a].ff;
                if(www<parent_lca[b].ss){
                    www=parent_lca[b].ss;
            ...
            if(www>w){
                ...
                for(int i=0; i<graph[AA].size(); i++){
                    if(graph[AA][i].ff==BB){graph[AA].erase(graph[AA].begin()+i); break;}
                }
                for(int i=0; i<graph[BB].size(); i++){
                    if(graph[BB][i].ff==AA){graph[BB].erase(graph[BB].begin()+i); break;}
                }
                graph[u].pb({v,w});
                graph[v].pb({u,w});
            }
