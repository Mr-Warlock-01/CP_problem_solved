...
void bfs(int x,  vector<int>graph[], vector<bool>&visit, vector<int>&visited_by){
    visit[x]=1; visited_by[x]++;
    queue<int>qq;   qq.push(x);
    while(!qq.empty()){
        int z=qq.front();   qq.pop();
        for(int i=0; i<graph[z].size(); i++){
            if(visit[graph[z][i]]==0){
                visit[graph[z][i]]=1;   visited_by[graph[z][i]]++;
                qq.push(graph[z][i]);
            }
        }
    }
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>visited_by(n+1);
    vector<bool>visit(n+1);
    vector<int>people(k);   cin>>people;
    ...
    for(int i=0; i<k; i++){\
        fill(all(visit),0);
        bfs(people[i], graph, visit, visited_by);
    }

    int ans=0;
    for(int i=0; i<=n; i++){
        if(visited_by[i]==k){ans++;}
    }
    db(visited_by)
