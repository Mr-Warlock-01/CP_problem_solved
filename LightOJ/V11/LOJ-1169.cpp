...
int xxx(int i, int flag, vector<vector<int>>&food, vector<vector<int>>&tower, vector<vector<int>>&dp){
    if(i==(n-1)){return food[flag][i];}
    if(dp[flag][i]!=-1){return dp[flag][i];}
    int a=food[flag][i] + xxx(i+1, flag, food, tower, dp);
    int b=food[flag][i] + tower[flag][i] + xxx(i+1, flag^1, food, tower, dp);
    return dp[flag][i]=min(a,b);
}


void pikachu(int T){         ///MAIN  (DRINK WATER)
    cin>>n;
    vector<vector<int>>food(2, vector<int>(n));         cin>>food;
    vector<vector<int>>tower(2, vector<int>(n-1));      cin>>tower;
    vector<vector<int>>dp(2, vector<int>(n,-1));    
    
    cout<<"Case "<<T<<": ";
    cout<<min(xxx(0,0,food,tower,dp) ,xxx(0,1,food,tower,dp))<<endl;
return;
}
