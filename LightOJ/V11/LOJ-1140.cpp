...
int n;
string a,b; 
ll dp[10][10][2][2][2]; //pos,num,start, lower, higher


ll calculate(int pos, bool lower, bool upper){
    ll small=0, big=0;
    if(!lower){small=0;}
    else{
        for(int i=pos+1; i<n; i++){small*=10; small+=(a[i]-'0');}
    }
    
    if(!upper){big=powMod(10,n-pos-1);  big--;}
    else{
        for(int i=pos+1; i<n; i++){   big*=10;    big+=(b[i]-'0');}
    }    
    return big-small+1;
}


ll xxx(int pos, int num, bool started, bool lower, bool upper){
    if(pos==n){return 0;}
    if(dp[pos][num][started][lower][upper]!=-1){ return dp[pos][num][started][lower][upper];}
    ...
    if(i==j){
        if((i==0) && started==1){
            ans+=calculate(pos, lower, upper);
            ans+=xxx(pos+1, i, 1, lower, upper);
        }
        else if(i==0){ans+=xxx(pos+1, i, 0, lower, upper);}
        else{ans+=xxx(pos+1, i, 1, lower, upper);}
    }
    else{
        for(int k=i; k<=j; k++){
            if((k==0) && started==1){ans+=calculate(pos, lower, 0);}
            if(k==i){       
                if(k!=0){   ans+=xxx(pos+1, k, 1, lower, 0);}
                else{       ans+=xxx(pos+1, k, started, lower, 0);}
            }
            else if(k==j){  ans+=xxx(pos+1, k, 1, 0, upper);}
            else{           ans+=xxx(pos+1, k, 1, 0, 0);}
        }
    }
    return dp[pos][num][started][lower][upper]= ans;   
}
