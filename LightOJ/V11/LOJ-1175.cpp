...
void bfs_fire(vector<vector<char>>&maze, vector<pair<int,int>>&fire, vector<vector<int>>&fire_catch, int n, int m){
    vector<vector<bool>>visit(n,vector<bool>(m));
    queue<pair<int,int>>qq;
    for(int i=0; i<fire.size(); i++){
        qq.push(fire[i]);
        visit[fire[i].ff][fire[i].ss]=1;
    }
    while(!qq.empty()){
        int y=qq.front().ff;
        int x=qq.front().ss;
        qq.pop();
        if(((x+1)<m) && (visit[y][x+1]==0) && (maze[y][x+1]!='#')){
            qq.push({y, x+1});  visit[y][x+1]=1;    fire_catch[y][x+1]=(fire_catch[y][x]+1);
        }
        if((x>0) && (visit[y][x-1]==0) && (maze[y][x-1]!='#')){
            qq.push({y, x-1});  visit[y][x-1]=1;    fire_catch[y][x-1]=(fire_catch[y][x]+1);
        }
        if(((y+1)<n) && (visit[y+1][x]==0) && (maze[y+1][x]!='#')){
            qq.push({y+1, x});  visit[y+1][x]=1;    fire_catch[y+1][x]=(fire_catch[y][x]+1);\
        }
        if((y>0) && (visit[y-1][x]==0) && (maze[y-1][x]!='#')){
            qq.push({y-1, x});  visit[y-1][x]=1;    fire_catch[y-1][x]=(fire_catch[y][x]+1);
        }
    }
}
...
void bfs_maze(vector<vector<char>>&maze, vector<vector<int>>&fire_catch, int n, int m, int jy, int jx, int &ans){
    vector<vector<bool>>visit(n,vector<bool>(m));
    queue<pair<int,int>>qq;
    queue<int>dis;

    qq.push({jy,jx});   dis.push(0);    visit[jy][jx]=1;
    while(!qq.empty()){
        int y=qq.front().ff;
        int x=qq.front().ss;
        int d=dis.front();
        qq.pop();   dis.pop();
        if((x==0) || (y==0) || ((x+1)==m) || ((y+1)==n)){ans=d+1; return;}

        if(((x+1)<m) && (visit[y][x+1]==0) && (maze[y][x+1]=='.') && ((d+1)<fire_catch[y][x+1])){
            qq.push({y, x+1});  visit[y][x+1]=1;    dis.push(d+1);
        }
        if((x>0) && (visit[y][x-1]==0) && (maze[y][x-1]=='.') && ((d+1)<fire_catch[y][x-1])){
            qq.push({y, x-1});  visit[y][x-1]=1;    dis.push(d+1);
        }
        if(((y+1)<n) && (visit[y+1][x]==0) && (maze[y+1][x]=='.') && ((d+1)<fire_catch[y+1][x])){
            qq.push({y+1, x});  visit[y+1][x]=1;    dis.push(d+1);
        }
        if((y>0) && (visit[y-1][x]==0) && (maze[y-1][x]=='.') && ((d+1)<fire_catch[y-1][x])){
            qq.push({y-1, x});  visit[y-1][x]=1;    dis.push(d+1);
        }
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<pair<int,int>>fire;      ///y,x
    vector<vector<int>>fire_catch(n,vector<int>(m));
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            ...
            else if(maze[i][j]=='J'){jy=i;  jx=j;}
        }
    }

    bfs_fire(maze, fire, fire_catch, n, m);
    int ans=0;
    bfs_maze(maze, fire_catch, n, m, jy, jx, ans);
