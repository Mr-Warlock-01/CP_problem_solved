...
void bfs(int p, int a, int b, vector<vector<int>>&moves){
    vector<vector<int>>v(n, vector<int>(m,LLMax));
    v[a][b]=0;
    queue<tuple<int,int,int>>qq;     qq.push({0, a, b});
    while(!qq.empty()){
        int z =get<0>(qq.front())+1;
        int xx=get<1>(qq.front());
        int yy=get<2>(qq.front());
        qq.pop();
        for(int i=0; i<8; i++){
            int x=xx+knight_x[i];
            int y=yy+knight_y[i];

            if((x>=0) && (x<n) && (y>=0) && (y<m) && (v[x][y]==LLMax)){
                v[x][y]=z;
                qq.push({z,x,y});
            }        }
    }
    ...
    for(int i=0; i<n; i++){
            for (int j=0; j<m; j++){
                if(moves[i][j]!=LLMax){
                    if(v[i][j]==LLMax){moves[i][j]=LLMax;}
                    else{moves[i][j]+=(v[i][j]+p-1)/p;}
                }
            }
        }
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    cin>>n>>m;
    vector<pair<int, pair<int,int>>>knight;
    ...
    for(int i=0; i<knight.size(); i++){
        bfs(knight[i].ff, knight[i].ss.ff, knight[i].ss.ss, moves);
    }
