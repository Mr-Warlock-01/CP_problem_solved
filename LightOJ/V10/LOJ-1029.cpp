...
int find_set(int x ,vector<pair<int,int>>&dsu){
    if(x==dsu[x].ff){return x;}
    return dsu[x].ff = find_set(dsu[x].ff, dsu);
}

void union_sets(int u, int v, vector<pair<int,int>>&dsu){
    u=find_set(u, dsu);
    v=find_set(v, dsu);
    if(u!=v){
        if(dsu[u].ss<dsu[v].ss){swap(u,v);}
        dsu[v].ff=u;
        dsu[u].ss+=dsu[v].ff;
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<pair<int,int>>dsu(n);    //parent,size;
    priority_queue<pair<int,int>,vector<pair<int,int>>,greater<pair<int,int>>>pq_min;  //cost,flag      min spaning tree
    ...
    while(!pq_min.empty()){
        int w= pq_min.top().ff;
        int u= nodes[pq_min.top().ss].ff;
        int v= nodes[pq_min.top().ss].ss;
        pq_min.pop();

        if(find_set(u, dsu)!= find_set(v, dsu)){
            cost_min+=w;
            union_sets(u, v, dsu);
        }
    }
    ...
    while(!pq_max.empty()){
        int w= pq_max.top().ff;
        int u= nodes[pq_max.top().ss].ff;
        int v= nodes[pq_max.top().ss].ss;
        pq_max.pop();
