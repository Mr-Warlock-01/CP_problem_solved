...
void dfs(int x, int d, vector<pair<int,int>>graph[], vector<int>&visit, int &lowest_cost, int n, int cost){
    if(d==n){
        if((graph[x][0].ff==1) && ((cost+graph[x][0].ss)<lowest_cost)){lowest_cost=cost+graph[x][0].ss;}
        else if((graph[x][1].ff==1) && ((cost+graph[x][1].ss)<lowest_cost)){lowest_cost=cost+graph[x][1].ss;}
        return;
    }
    visit[x]=1;
    for(int i=0 ;i<graph[x].size(); i++){
        if(visit[graph[x][i].ff]==0){
            dfs(graph[x][i].ff, d+1, graph, visit, lowest_cost, n, cost+graph[x][i].ss);
        }
    }
    visit[x]=0;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    int lowest_cost=LONG_LONG_MAX;
    dfs(1, 1, graph, visit, lowest_cost, n, 0);
    cout<<"Case "<<T<<": "<<lowest_cost<<endl;
}
