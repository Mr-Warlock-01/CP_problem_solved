...
void dfs(int x, vector<vector<int>>&graph, vector<int>&visit){
    visit[x]++;
    for(int i=0; i<graph[x].size(); i++){
        if(visit[graph[x][i]]==0){ dfs(graph[x][i], graph, visit);}
        else if(visit[graph[x][i]]==1){ans=0; return;}
    }
    visit[x]++;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int m;  cin>>m;
    int n=1;
    vector<vector<int>>graph;   graph.pb({});
    map<string,int>mp;
    while(m--){
        string s1,s2;   cin>>s1>>s2;
        if(mp[s1]==0){mp[s1]=n;     graph.pb({});   n++;}
        if(mp[s2]==0){mp[s2]=n;     graph.pb({});   n++;}
