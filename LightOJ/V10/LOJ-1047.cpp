...
int xxx(int x, int y, vector<vector<int>>&v, vector<vector<int>>&dp){
    if(x==n){return 0;}
    if(y<0 || y>2){return LLMax;}
    if(dp[x][y]!=-1){return dp[x][y];}
    return dp[x][y]= v[x][y]+min({xxx(x+1, y-2, v, dp), xxx(x+1, y-1, v, dp), xxx(x+1, y+1, v, dp), xxx(x+1, y+2, v, dp)});
}

void pikachu(int T){         ///MAIN  (DRINK WATER)
    cin>>n;
    vector<vector<int>>v(n, vector<int>(3));        cin>>v;
    vector<vector<int>>dp(n, vector<int>(3,-1));

    cout<<"Case "<<T<<": ";
    cout<<min({xxx(0, 0, v, dp), xxx(0, 1, v, dp), xxx(0, 2, v, dp)})<<endl;
}
