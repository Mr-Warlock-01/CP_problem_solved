...
void build_sparse(){
    for(int z=1; z<10; z++){
        for(int i=0; i<n; i++){
            for(int j=0; j<n; j++){
                int check=max(i,j)+(1<<z);
                if(check>n){break;}
                int aa=sp[z-1] [i] [j];
                int bb=sp[z-1] [i] [j+(1<<(z-1))];
                int cc=sp[z-1] [i+(1<<(z-1))] [j];
                int dd=sp[z-1] [i+(1<<(z-1))] [j+(1<<(z-1))];
                sp[z][i][j]=max({aa,bb,cc,dd});
            }
        }
    }
}
...
void solve(int T){            ///   (DRINK WATER)
    ...
    while(q--){
        ...
        int aaa= sp[k] [i1] [j1];
        int bbb= sp[k] [i1] [j2-(1<<k)+1];
        int ccc= sp[k] [i2-(1<<k)+1] [j1];
        int ddd= sp[k] [i2-(1<<k)+1] [j2-(1<<k)+1];

        int ans=max({aaa,bbb,ccc,ddd});
        cout<<ans<<endl;
    }
}
