...
bool check_vowel(string &s, int i){
    if(s[i]=='A' || s[i]=='E' || s[i]=='I' || s[i]=='O' || s[i]=='U'){return 1;}
    return 0;
}

int good_bad_mixed(vector<vector<vector<int>>>&dp, string &s, int i, int vow, int conso){
    if(vow>=3 || conso>=5){ return 0;}
    if(i>=n){return 1;}
    if(dp[i][vow][conso]!=-1){return dp[i][vow][conso];}

    if(s[i]=='?'){
        int v=good_bad_mixed(dp, s, i+1, vow+1, 0);
        int c=good_bad_mixed(dp, s, i+1, 0, conso+1);
        if(v!=c || v==2 || c==2){
            return dp[i][vow][conso]=2;
        }
        else{
            return dp[i][vow][conso]=v;
        }
    }

    if(check_vowel(s, i)){
        return dp[i][vow][conso]=good_bad_mixed(dp, s, i+1, vow+1, 0);
    }
    else{
        return dp[i][vow][conso]=good_bad_mixed(dp, s, i+1, 0, conso+1);
    }
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    ///0 bad, 1 good, 2 mixed

    cout<<"Case "<<T<<": ";
    int ans=good_bad_mixed(dp, s, 0, 0, 0);
