...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    while(m--){
        ...
        int w;  cin >>w;
        if(mp[s1]==0){mp[s1]=flag;  flag++;}
        if(mp[s2]==0){mp[s2]=flag;  flag++;}
        pq.push({w, mp[s1], mp[s2]});
    }

    vector<int>parent(flag);        for(int i=0; i<flag; i++){parent[i]=i;}
    vector<int>dsu_size(flag,1);
    int ans=0;
    ...
    while(!pq.empty()){
        ...
        int v = get<2>(pq.top());
        pq.pop();
        if(find_set(u, parent, dsu_size)!= find_set(v, parent, dsu_size)){
            ans+=w;
            union_sets(u, v, parent, dsu_size);
        }
    }

    int parent_count=0;
    for(int i=1; i<flag; i++){
        if(parent[i]==i){parent_count++;}
    }

    cout<<"Case "<<T<<": ";
    if(parent_count==1){cout<<ans<<endl;}
