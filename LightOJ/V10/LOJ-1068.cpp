...
bool fff(string &s){
    ...
    if(!(dig_sum%k) && !(num%k)){return 1;}
return 0;
}

int xxx(int pos, int sum, int num, bool flag, string &s){
    if(pos<0){
        if((sum==0) && (num==0)){return 1;}
        return 0;
    }
    if(dp[pos][sum][num][flag]!=-1){return dp[pos][sum][num][flag];}

    int ans=0,high=9;
    if(flag==1){high=s[pos]-'0';}
    for(int i=0; i<=high; i++){

        if((i==high) && (flag==1)){ans+=xxx(pos-1, ((sum*10)+i)%k, (num+i)%k, 1,s);}
        else{                           ans+=xxx(pos-1, ((sum*10)+i)%k, (num+i)%k, 0,s);}
    }
    return dp[pos][sum][num][flag]=ans;
}



//See previous submission for better understanding
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    if(k<91){
        ...
        int ans2=xxx(b.size()-1,0,0,1,b);
        memset(dp,-1,sizeof(dp));
        int ans1=xxx(a.size()-1,0,0,1,a);
        ans=ans2-ans1;
        if(fff(a)){ans++;}
    }
