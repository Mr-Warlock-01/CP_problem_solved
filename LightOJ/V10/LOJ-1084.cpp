...
int xxx(int e, vector<int>&v, vector<int>&dp){ 
    if(e==n){return 0;}
    ...
    if((point-e)>2){ans=min(ans, xxx(point, v, dp));}
    if((point-e-1)>2){ans=min(ans, xxx(point-1, v, dp));}
    if((point-e-2)>2){ans=min(ans, xxx(point-2, v, dp));}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    cin>>n>>k;
    vector<int>v(n+1);    for(int i=0; i<n; i++){cin>>v[i];}
    vector<int>dp(n+2, -1);
    sort(all(v));
