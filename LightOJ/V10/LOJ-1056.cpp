...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    cin>>a>>ch>>b;
    deci(12);
    double b_times_x=a/b;
    
    double l=0, r=100;
    double length,width;
    
    int BS_on_decimal=100;
    while(BS_on_decimal--){
        double mid=(l+r)/2;
        double w= (2*mid)/ (sqrt(1+(b_times_x*b_times_x)));
        double theta=2*asin((w/2)/mid);
        double ans=(2*theta*mid)+(2*w*b_times_x);
        if(ans>=400){
            r=mid-1; length=w*b_times_x; width=w;
            db(mid);    db(ans);
        }
        else{l=mid+1;}
    }
    cout<<"Case "<<T<<": "<<length<<" "<<width<<endl;
}
