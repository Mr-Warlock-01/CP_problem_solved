...
void build(vector<pair<int,int>>&v, vector<pair<int,int>>&tree, int node, int st, int en){
    if(st==en){tree[node].ff = v[st].ff;    tree[node].ss=v[st].ss;     return;}
    int mid=((en-st)/2)+st;
    build(v, tree, 2*node, st, mid);
    build(v, tree, 2*node+1, mid+1, en);
    tree[node].ss=tree[2*node].ss+tree[2*node+1].ss;
    return;
}

void update(vector<pair<int,int>>&v, vector<pair<int,int>>&tree, int node, int st, int en, int ind, int val){
    if(st==en){tree[node].ff=val;      tree[node].ss=1; return;}
    int mid=((en-st)/2)+st;
    if(st<=ind && mid>=ind){    update(v, tree, 2*node,st, mid, ind, val);}
    else{                       update(v, tree, 2*node+1, mid+1, en, ind, val);}
    tree[node].ss=tree[2*node].ss + tree[2*node+1].ss;
    return;
}

int query(vector<pair<int,int>>&tree, int node, int st, int en, int posi){
    if(st==en){tree[node].ss=0; return tree[node].ff;}
    int mid=((en-st)/2)+st;
    int x;
    if(tree[2*node].ss>= posi){x=query(tree, 2*node, st, mid, posi);}
    else{x=query(tree, 2*node+1, mid+1, en, posi-tree[2*node].ss );}
    tree[node].ss=tree[2*node].ss + tree[2*node+1].ss;
    return x;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n,q;    cin>>n>>q;
    vector<pair<int,int>>v(n+q), tree(4*(n+q));
    for(int i=0; i<n; i++){cin>>v[i].ff;   v[i].ss=1;}
    build(v,tree,1,0,n+q-1);
    int kkk=q;
    int total_element=n;
    int ind=n;
    cout<<"Case "<<T<<":"<<endl;
    while(kkk--){
        ...
        cin>>ch>>val;
        if(ch=='a'){
            update(v, tree, 1, 0, n+q-1, ind, val);
            ind++;  total_element++;
        }
        else{
            if(total_element<val){cout<<"none"<<endl;}
            else{
                cout<<query(tree, 1, 0, n+q-1, val)<<endl;
                total_element--;
            }
        }
    }
