...
void dfs(int x, vector<bool>&negative_cycle, vector<bool>&visit, vector<int>graph[]){
    visit[x]=1; negative_cycle[x]=1;
    for(int i=0; i<graph[x].size(); i++){
        if(!visit[graph[x][i]]){
            dfs(graph[x][i], negative_cycle, visit, graph);
        }
    }
return;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<m; i++){
        graph[get<0>(edge[i])].pb(get<1>(edge[i]));
    }

    vector<int>weight(n+1, MOD);   weight[1]=0;
    ...
    for(int i=1; i<n; i++){
        for(int j=0; j<m;  j++){
            ...
            int w=get<2>(edge[j]);
            if(weight[a]!=MOD){
                weight[b]=min(weight[b], weight[a]+w);
            }
        }
    }

    vector<bool>negative_cycle(n+1);
    ...
    for(int i=1; i<=n; i++){
        if(negative_cycle[i]==1 && visit[i]==0){
            dfs(i, negative_cycle, visit, graph);
        }
    }
    ...
    while(q--){
        int x;  cin>>x;
        if((negative_cycle[x]==1) || (weight[x]<3) || (weight[x]==MOD)){cout<<"?"<<endl;}
        else{cout<<weight[x]<<endl;}
    }
return;
}
