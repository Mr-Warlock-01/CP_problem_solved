...
void bridge(int x, int p, vector<int>graph[], vector<bool>&visit, vector<int>&in, vector<int>&low, vector<pair<int,int>>&ans){
        visit[x]=1;
        in[x]=low[x]=(timer++);
        for(int i=0; i<graph[x].size(); i++){
            if(graph[x][i]==p){continue;}
            else if(visit[graph[x][i]]){
                low[x]=min(low[x], in[graph[x][i]]);
            }
            else{
                bridge(graph[x][i], x, graph, visit, in, low, ans);
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<n; i++){
        if(!visit[i]){
            bridge(i, -1, graph, visit, in, low, ans);
        }
    }
    sort(all(ans));
    cout<<"Case "<<T<<": "<<endl;
