...
void build_min(int arr[], int tree_min[], int node, int st, int en){
    if(st==en){tree_min[node]=arr[st];  return;}
    int mid =((en-st)/2)+st;
    build_min(arr, tree_min, 2*node, st, mid);
    build_min(arr, tree_min, 2*node+1, mid+1, en);
    tree_min[node]= min(tree_min[2*node], tree_min[2*node+1]);
}

void build_max(int arr[], int tree_max[], int node, int st, int en){
    if(st==en){tree_max[node]=arr[st];  return;}
    int mid =((en-st)/2)+st;
    build_max(arr, tree_max, 2*node, st, mid);
    build_max(arr, tree_max, 2*node+1, mid+1, en);
    tree_max[node]= max(tree_max[2*node], tree_max[2*node+1]);
}

int query_min (int tree[], int node, int st, int en, int l, int r){
    if(st>r || en<l){return LONG_MAX;}
    else if(st>=l && en<=r){return tree[node];}
    int mid =((en-st)/2)+st;
    int q1= query_min(tree, 2*node, st, mid, l, r);
    int q2= query_min(tree, 2*node+1, mid+1, en, l, r);
    return min(q1, q2);
}
int query_max(int tree[], int node, int st, int en, int l, int r){
    if(st>r || en<l){return 0;}
    else if(st>=l && en<=r){return tree[node];}
    int mid =((en-st)/2)+st;
    int q1= query_max(tree, 2*node, st, mid, l, r);
    int q2= query_max(tree, 2*node+1, mid+1, en, l, r);
    return max(q1, q2);
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    int tree_min[4*n];
    int tree_max[4*n];
    build_min(arr, tree_min, 1, 0, n-1);
    build_max(arr, tree_max, 1, 0, n-1);

    int ans=0;
    for(int i=0; i<=(n-k); i++){
        int x= query_min(tree_min, 1 , 0, n-1, i, i+k-1);
        int y= query_max(tree_max, 1 , 0, n-1, i, i+k-1);
        ans=max(ans, y-x);
    }
