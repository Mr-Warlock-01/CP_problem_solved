...
void bfs(vector<pair<int,int>>graph[], vector<int>&ans){
    ...
    while(!pq.empty()){
        ...
        if(ans[x]==LLMax){
            ...
            for(int i=0; i<graph[x].size(); i++){
                if(ans[graph[x][i].ff]==LLMax){
                    pq.push({d+graph[x][i].ss, graph[x][i].ff});
                }
            }
        }
    }
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    cout<<"Case "<<T<<": ";
    if(ans[n]==LLMax){cout<<"Impossible"<<endl;}
