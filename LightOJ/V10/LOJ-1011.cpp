...
int xxx(int rank, int mask, vector<vector<int>>&v, vector<vector<int>>&dp){
    if(rank==n){return 0;}
    if(dp[rank][mask]!=-1){return dp[rank][mask];}
    
    int ans=0;
    for(int i=0; i<n; i++){
        if((mask & (1<<i))==0){
            ans=max(ans, v[rank][i]+ xxx(rank+1, mask | (1<<i), v, dp));
        }
    }
    return dp[rank][mask]=ans;
}


void pikachu(int T){         ///MAIN  (DRINK WATER)
    cin>>n;
    vector<vector<int>>v(n, vector<int>(n));    cin>>v;
