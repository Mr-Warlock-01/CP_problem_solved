...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    priority_queue<pair<int,int>,vector<pair<int,int>>,greater<pair<int,int>>>pq;  //cost,flag
    ...
    while(!pq.empty()){
        int w=pq.top().ff;
        int u= nodes[pq.top().ss].ff;
        int v= nodes[pq.top().ss].ss;
        pq.pop();
    ...
    int checking=0;
    for(int i=0; i<n; i++){
        if(dsu[i].ff==i){checking++;}
    }

    cout<<"Case "<<T<<": ";
    if(checking==1){cout<<total_cable-cable_use<<endl;}
    else{cout<<"-1"<<endl;}
}
