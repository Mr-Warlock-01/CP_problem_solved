...
void solve(int T){
    ...
    int M=10;
    for(int i=1; i<m; i++){M*=10;}
    cout<<"Case "<<T<<": ";
    if(n==0){cout<<a%M<<endl; return;}
    vector<vector<int>>Seq(2,vector<int>(2));
    vector<vector<int>>Pat(2,vector<int>(2));  
    ...
    Seq=matrix_pow(Seq, n-1, 2, M); 
    Pat=matrix_mul(Seq, Pat, 2, 2, 1,M);
