...
void articlar_point_check(int x, int p, vector<int>graph[], vector<bool>&visit, vector<bool>&point, vector<int>&in_time, vector<int>&low_time){
    visit[x]=1;
    ...
    for(int u:graph[x]){
        if(u==p){continue;}
            ...
            articlar_point_check(u, x, graph, visit, point, in_time, low_time);
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=1; i<=n; i++){
        if(!visit[i]){
            articlar_point_check(i, -1, graph, visit, point, in_time, low_time);
        }
    }
