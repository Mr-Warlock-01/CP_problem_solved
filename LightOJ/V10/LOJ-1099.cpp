...
//double visited bfs
void bfs(vector<pair<int,int>>graph[], vector<int>&visit, vector<pair<int,int>>&weight){
    ...
    while(!pq.empty()){
        ...
        int x=pq.top().ss;
        pq.pop();
        if(visit[x]==2){continue;}
        else if(visit[x]==0){
            visit[x]++;     weight[x].ff=d;
        }
        else if((visit[x]==1) && (weight[x].ff==d)){continue;}
        else{visit[x]++;     weight[x].ss=d;}

        for(int i=0; i<graph[x].size(); i++){
            if(visit[graph[x][i].ff]!=2){pq.push({d+graph[x][i].ss ,graph[x][i].ff});}
        }
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<pair<int,int>>weight(n+1,{LLMax, LLMax});
    while(r--){
        int u,v,w;    cin>>u>>v>>w;
        graph[u].pb({v,w});
        graph[v].pb({u,w});
    }
    bfs(graph, visit, weight);      //start with 1
    cout<<"Case "<<T<<": ";
    cout<<weight[n].ss<<endl;
}
