...
bool palindrom_check(int i, int j, vector<vector<int>>&palindrom, string &s){
    if(i>=j){return 1;}
    if(palindrom[i][j]!=-1){return palindrom[i][j];}
    bool x= 0;
    if(s[i]==s[j]){x=palindrom_check(i+1, j-1, palindrom, s);}
    return palindrom[i][j]=x;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    string s;   cin>>s;
    int sz=s.size();
    vector<vector<int>>palindrom(sz, vector<int>(sz,-1));
    vector<int>dp(sz,10000);

    for(int j=0; j<sz; j++){
        for(int i=0; i<=j ; i++){
            if(palindrom_check(i, j, palindrom, s)){
                if(i==0){dp[j]=1;}
