...
int xxx(int score, int dice, vector<vector<int>>&dp){
    if(dice==n){return (score>=x);}
    if(dp[score][dice]!=-1){return dp[score][dice];}
    
    int ans=0;
    for(int i=1; i<=6; i++){
        ans+=xxx(score+i, dice+1, dp);
    }
    return dp[score][dice]=ans;
}

void pikachu(int T){         ///MAIN  (DRINK WATER)
    cin>>n>>x;
    vector<vector<int>>dp(6*n+1, vector<int>(6*n+1, -1));

    int p=xxx(0, 0, dp);
    int q=powMod(6,n,LONG_LONG_MAX);
