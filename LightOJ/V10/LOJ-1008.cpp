...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    if((x*x)==n){
        ...
        ll mid= (z+(x*x)+1)/2;
        db(mid)
        if(mid==n){cout<<x+1<<" "<<x+1<<endl;}
        else if(x%2==0){
            if(mid>n){cout<<x+1<<" "<<n-(x*x)<<endl;}
            else{cout<<z-n+1<<" "<<x+1<<endl;}
        }
        else{
            if(mid<n){cout<<x+1<<" "<<z-n+1<<endl;}
            else{cout<<n-(x*x)<<" "<<x+1<<endl;}
        }
    }
return;
}
