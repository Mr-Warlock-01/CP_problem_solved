...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<vector<int>>visit(m,vector<int>(n));     //2d vector
    int ans=0;
    queue<int> xx,yy;    xx.push(x);    yy.push(y);
    ///BFS
    visit[y][x]=1;  ans++;
    while(!xx.empty() && !yy.empty()){
        ...
        if(((x+1)<n) && (visit[y][x+1]==0) && (arr[y][x+1]=='.')){
            ans++; xx.push(x+1);    yy.push(y);     visit[y][x+1]=1;
        }
        if((x>0) && (visit[y][x-1]==0) && (arr[y][x-1]=='.')){
            ans++; xx.push(x-1);    yy.push(y);     visit[y][x-1]=1;
        }
        if(((y+1)<m) && (visit[y+1][x]==0) && (arr[y+1][x]=='.')){
            ans++; xx.push(x);    yy.push(y+1);     visit[y+1][x]=1;
        }
        if((y>0) && (visit[y-1][x]==0) && (arr[y-1][x]=='.')){
            ans++; xx.push(x);    yy.push(y-1);     visit[y-1][x]=1;
        }
    }
    cout<<"Case "<<T<<": ";
