...
void dfs(int x, vector<int>graph[], vector<bool>&visit, int n){
    visit[x]=1; total_visited++;
    for(int i=0; i<graph[x].size(); i++){
        if(visit[graph[x][i]]==0){
            dfs(graph[x][i], graph, visit, n);
        }
    }
    unfrnd_pair+=(n-total_visited);
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    cout<<"Case "<<T<<": ";
    cout<<total_visited-1<<" "<<unfrnd_pair<<endl;
}
