...
int xxx(int pos, int flag, vector<int>&v, vector<vector<int>>&dp){
    if((flag==0) && (pos>=n)){return 0;}
    if((flag==1) && (pos>=(n-1))){return 0;}
    if(dp[flag][pos]!=-1){return dp[flag][pos];}

    int a= xxx(pos+1, flag, v, dp);
    int b= xxx(pos+2, flag, v, dp)+v[pos];
    return dp[flag][pos]=max(a,b);
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    cout<<"Case "<<T<<": ";
    cout<<max(xxx(1,0,v,dp),xxx(0,1,v,dp))<<endl;
return;
}
