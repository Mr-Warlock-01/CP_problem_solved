...
bool bfs(int x, vector<int>graph[], vector<int>&visit,  vector<int>&ind_money, int equal_money){
    vector<int>v;
    queue<int>qq;
    qq.push(x); v.pb(x);    visit[x]=1;
    while(!qq.empty()){
        int z=qq.front();       qq.pop();
        for(int i=0 ;i<graph[z].size(); i++){
            if(visit[graph[z][i]]==0){
                visit[graph[z][i]]=1;
                v.pb(graph[z][i]);
                qq.push(graph[z][i]);
            }
    ...
    int ans=0, sz=v.size();
    for(int i=0; i<sz; i++){ans+=ind_money[v[i]];}
    if(((ans%sz)==0) && ((ans/sz)==equal_money)){return 1;}
    return 0;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n,m;    cin>>n>>m;
    vector<int>ind_money(n);    cin>>ind_money;
    vector<int>visit(n);
    vector<int>graph[n];
    while(m--){
        int u,v;    cin>>u>>v;
    ...
    bool flag=1;
    int sum=0;
    for(int i=0; i<n; i++){sum+=ind_money[i];}
    if(sum%n!=0){flag=0;}
    int equal_money=sum/n;
    for(int i=0; i<n; i++){
        if(flag==0){break;}
        if(visit[i]==0){
            if(bfs(i, graph, visit, ind_money, equal_money)==0){flag=0;}
        }
    }
