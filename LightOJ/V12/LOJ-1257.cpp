...
void dfs(int x, vector<int>&visit, vector<pair<int ,int>>graph[], vector<int>&ans) {
    for(int i=0; i<graph[x].size(); i++){
        if(visit[graph[x][i].ff]==0){
            visit[graph[x][i].ff]=1;
            weight_sum+=graph[x][i].ss;
            ans[graph[x][i].ff]=max(ans[graph[x][i].ff], weight_sum);
            if(max_weight_sum<weight_sum){
                max_weight_sum = weight_sum;
                corner_point=graph[x][i].ff;
            }
            dfs(graph[x][i].ff, visit, graph, ans);
            weight_sum-=graph[x][i].ss;
            visit[graph[x][i].ff]=0;
        }
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>visit(n), ans(n);

    corner_point=0; weight_sum=0; max_weight_sum=0;
    ...
    int ok=3;
    while(ok--){
        int start_from=corner_point;
        visit[start_from]=1;
        weight_sum=0;    max_weight_sum=0;
        dfs(corner_point, visit, graph, ans);
        visit[start_from]=0;
    }
    cout<<"Case "<<T<<":"<<endl;
