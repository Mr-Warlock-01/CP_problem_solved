...
int bfs(int xx, int yy, vector<vector<char>>&v, int n, int m){
    vector<vector<bool>>visit(n,vector<bool>(m));
    queue<pair<int,int>>qq;     //y,x
    queue<int>dd;               //distace
    qq.push({yy,xx});     dd.push(0);
    ...
    while(!qq.empty()){
        ...
        if(v[y+1][x]=='h'){return (d+1);}
        else if((v[y+1][x]!='#') && (v[y+1][x]!='m') && (visit[y+1][x]==0)){ qq.push({y+1,x});  dd.push(d+1);   visit[y+1][x]=1;}

        if(v[y-1][x]=='h'){return (d+1);}
        else if((v[y-1][x]!='#') && (v[y-1][x]!='m') && (visit[y-1][x]==0)){ qq.push({y-1,x});  dd.push(d+1);   visit[y-1][x]=1;}

        if(v[y][x+1]=='h'){return (d+1);}
        else if((v[y][x+1]!='#') && (v[y][x+1]!='m') && (visit[y][x+1]==0)){ qq.push({y,x+1});  dd.push(d+1);   visit[y][x+1]=1;}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            if(v[i][j]=='a'){xa=j; ya=i;}
            else if(v[i][j]=='b'){xb=j; yb=i;}
            else if(v[i][j]=='c'){xc=j; yc=i;}
        }
    }
    int ans1=bfs(xa, ya, v, n, m);
    int ans2=bfs(xb, yb, v, n, m);
    int ans3=bfs(xc, yc, v, n, m);
