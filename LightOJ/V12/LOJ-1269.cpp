...
void query_max(int x) { // "s" is a prefix of some element or not
    TrieNode *cur = root;
    int ans=0;
    for(int i=N; i>=0; i--) {
        bool idx=1&(x>>i);
        if(cur->nxt[idx^1]!=NULL){cur=cur->nxt[idx^1]; ans|=(1<<i);}
        else{cur=cur->nxt[idx];}
    }
    ans_max=max(ans,ans_max);
return;
}

void query_min(int x) { // "s" is a prefix of some element or not
    TrieNode *cur = root;
    int ans=0;
    for(int i=N; i>=0; i--) {
        bool idx=1&(x>>i);  
        if(cur->nxt[idx]!=NULL){cur=cur->nxt[idx];}
        else{cur=cur->nxt[idx^1]; ans|=(1<<i);}
    }
