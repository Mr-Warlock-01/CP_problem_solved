...
int kinght_x[]={2,  2, -2, -2, 1, -1,  1, -1};
int kinght_y[]={1, -1,  1, -1, 2,  2, -2, -2};

int bfs(int x_s, int y_s, int x_e, int y_e){
    vector<vector<bool>>visit(8, vector<bool>(8));
    queue<pair<int,int>>qq;
    queue<int>moves;
    if(x_s==x_e && y_s==y_e){return 0;}
    qq.push({x_s, y_s});
    moves.push(0);
    visit[x_s][y_s]=1;
    while(!qq.empty()){
        ...
        for(int z=0; z<8; z++){
            ...
            if((i>=0 && i<8) && (j>=0 && j<8) && (visit[i][j]==0)){
                visit[i][j]=1;
                if(i==x_e && j==y_e){return (d+1);}
                else{
                    qq.push({i,j}); moves.push(d+1);
                }
            }
        }
    }
