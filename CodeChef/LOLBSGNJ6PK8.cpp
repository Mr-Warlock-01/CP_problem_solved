...
void solve(int T){            ///   (DRINK WATER)
    ...
    vector<ll>M_prime(prime_no.size());
    ...
    for(int i=0; i<n; i++){
        ...
        for(int j=0; j<500; j++){
            if(z%prime_no[j]==0){
                int cnt=1;
                z/=prime_no[j];
                while(z%prime_no[j]==0){
                    z/=prime_no[j];
                    cnt++;
                }
                flag_ans=(flag_ans*(cnt+1+M_prime[j]))%MOD;
                flag_ans=(flag_ans*powMod(1+M_prime[j],MOD-2,MOD))%MOD;
            }
        }
        if(z!=1){
            flag_ans=(flag_ans*powMod((m/z)+1,MOD-2,MOD))%MOD;
            flag_ans=(flag_ans*((m/z)+2))%MOD;
        }
        ans[i]=flag_ans;
    }
    cout<<ans<<endl;
