...
int sz[N];
int color[N];
priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>>>ans[N];
int nn=0;
...
int query(int node){
    ...
    int res=LLMax;
    while(1){
        while(!color[ans[x].top().ss]){
            ans[x].pop();
        }
        res=min(res, find_dis(node,x)+ans[x].top().ff);
        if(x==par[x]){break;}
        x=par[x];
    }
    return res;
...
void update(int node){
    ...
    while(1){
        ...
        ans[x].push({find_dis(node,x),node});
        if(x==par[x]){break;}
        x=par[x];
    }
}
...
void solve(int T){
    ...
    color[0]=1;
    for(int i=1; i<=n; i++){ans[i].push({LLMax,0});}
