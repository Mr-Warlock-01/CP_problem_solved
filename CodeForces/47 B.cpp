...
void pikachu(){         //MAIN
    ...
    s="";
    for(int i=0; i<3; i++){if(v[i]==0){s+=(char)('A'+i); break;} }
    for(int i=0; i<3; i++){if(v[i]==1){s+=(char)('A'+i); break;} }
    for(int i=0; i<3; i++){if(v[i]==2){s+=(char)('A'+i); break;} }
    if((s[0]==s[1]) || (s[1]==s[2]) || (s[0]==s[2])|| s.size()!=3){cout<<"Impossible";}
    else{cout<<s;}
}
