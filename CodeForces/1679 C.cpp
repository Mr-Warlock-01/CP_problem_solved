...
void pikachu(){         //MAIN
    ...
    while(q--){
        ...
        if(t==1){
            cin>>a>>b;
            if(R[a]==0){Row.insert(a);}
            if(C[b]==0){Column.insert(b);}
            R[a]++; C[b]++;
        }
        else if(t==2){
            cin>>a>>b;
            R[a]--; C[b]--;
            if(R[a]==0){Row.erase(a);}
            if(C[b]==0){Column.erase(b);}
        }
        else{
            cin>>a>>b>>c>>d;
            bool flag=0;
            if( (Row.order_of_key(c+1) - Row.order_of_key(a)) == ((c+1)-a) ){flag=1;}
            if( (Column.order_of_key(d+1) - Column.order_of_key(b)) == ((d+1)-b) ){flag=1;}
            if(flag==1){YES;}
            else{NO;}
        }
    }
