...
bool is_Palindrome(string s) {int i=0, j=s.size()-1; for(i, j; i<=j; i++, j--){if(s[i]!=s[j]) return 0;} return 1;}
bool is_Prime(int n) {for(int i=2; i*i<= n; i++)if ((n%i)==0){return false;} return true;}
bool is_PowerofTwo(int n){return (n && !(n & (n - 1)));}
...
int digit_Count(int n) {return floor(log10(n) + 1);}
int digit_Sum(int n) {int sum=0; for(int i=n; i>0; i/=10){sum+=(i%10);} return sum;}
