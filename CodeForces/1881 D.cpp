...
signed main(){
    ...
    int x=1000000;  int sq_x=sqrt(x);

    vector<int>prime_check(sq_x+1);
    vector<int>prime_no;
    prime_no.push_back(2);

    for(int i=3; i<sq_x; i=i+2){
        if(prime_check[i]==1){continue;}
        else{prime_no.push_back(i);}
        for(int j=3*i; j<sq_x; j+=(2*i)){
            prime_check[j]=1;
        }
    }
    int sz=prime_no.size();
    ...
    while(tc--){
        int n;  cin>>n;
        vector<int>v(n);    map<int,int>m;      bool flag=1;
        for(int i=0; i<n ;i++){cin>>v[i];}
        sort(all(v));
        if(v[0]==v[n-1]){cout<<"YES"<<endl; continue;}
        for(int i=0; i<n; i++){
            for(int j=0; j<sz;  j++){
                while(v[i]%prime_no[j]==0){
                    m[prime_no[j]]++;
                    v[i]/=prime_no[j];
                }
                if(v[i]==1){break;}
            }
            if(v[i]!=1){flag=0; break;}
        }

        for(int i=0; i<sz; i++){
            if(m[prime_no[i]]%n!=0){flag=0; break;}
        }
