...
int n;  
vector<vector<int>>dp(3000, vector<int>(3000,LLMax));

int xxx(int pos, int pre, vector<pair<int,int>>&v){
    if(pos>=n){return 0;}
    if(dp[pos][pre]!=LLMax){return dp[pos][pre];}

    int x=xxx(pos+1,pos,v) + v[pos].ss;
    int y=xxx(pos+1,pre,v) + (v[pos].ff-v[pre].ff);
