...
void bfs(int x, vector<pair<int,int>>graph[], vector<int>&cost, vector<int>&comes_from){
    priority_queue<pair<int,int>>qq;        //(-weight),node
    qq.push({0,x});    cost[1]=0;
    while(!qq.empty()){
        int d=-qq.top().ff;
        int z=qq.top().ss;
        qq.pop();
        for(int i=0; i<graph[z].size(); i++){
            if((d+graph[z][i].ss)<cost[graph[z][i].ff]){
                qq.push({-(d+graph[z][i].ss), graph[z][i].ff});
                cost[graph[z][i].ff]=(d+graph[z][i].ss);
                comes_from[graph[z][i].ff]=z;
            }
        }
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>cost(n+1, LONG_LONG_MAX);
    vector<int>comes_from(n+1);
    int k=m;
    while(k--){
        int u,v,w;  cin>>u>>v>>w;
        graph[u].pb({v,w});
    ...
    bfs(1, graph, cost, comes_from);
    if(cost[n]==LONG_LONG_MAX){cout<<"-1";}
    else{
        vector<int>ans; ans.pb(n);
        int x=n;
        while(x!=1){x=comes_from[x];    ans.pb(x);}
