...
bool xxx(vector<int> &zero,vector<int> &one ,vector<int> &two, int zero_sz, int one_sz, int two_sz, int socket_len){
    for(int i=0; i<zero_sz; i++){socket_len-=zero[i];}
    int ok=min(one_sz, two_sz);
    for(int i=0; i<ok; i++){socket_len-=(one[i]+two[i]);}
    bool flag=0;
    for(int i=ok; i<one_sz; i++){
        if(flag==0){flag=1; socket_len-=(one[i]+2);}
        else{flag=0; socket_len-=(one[i]-1);}
    }
    for(int i=ok; i<two_sz; i++){
        socket_len-=(two[i]+1);
    }
    if(socket_len>=0){return 1;}
    else{return 0;}
}

void pikachu(){         ///MAIN  (DRINK WATER)
    int n,s; cin>>n>>s;
    vector<int>v(n);    cin>>v;
    if(s==1 || n==1){cout<<"1"<<endl; return;}
    else if(s==2 && n>=2){cout<<"2"<<endl; return;}
    s-=2;   s*=3;

    sort(all(v));   v.pop_back();   v.pop_back();

    vector<int>mod_1(n-2), mod_2(n-2);
    vector<int>zero,one,two;

    if(v[0]%3==1){mod_1[0]=1; one.push_back(v[0]);}
    else if(v[0]%3==2){mod_2[0]=1;two.push_back(v[0]);}
    else{ zero.push_back(v[0]);}

    for(int i=1; i<v.size(); i++){
        if((v[i]%3)==0){mod_1[i]=mod_1[i-1];      mod_2[i]=mod_2[i-1];  zero.push_back(v[i]);}
        else if((v[i]%3)==1){mod_1[i]=mod_1[i-1]+1;      mod_2[i]=mod_2[i-1];   one.push_back(v[i]);}
        else{mod_1[i]=mod_1[i-1];      mod_2[i]=mod_2[i-1]+1; two.push_back(v[i]);}
    }
    ...
    while(l<=r){
        int mid= ((r-l)/2)+l;
        int sz_zero=mid+1-( mod_1[mid]+mod_2[mid]);
        int sz_one=mod_1[mid];
        int sz_two=mod_2[mid];
        
        bool haha=xxx(zero, one, two, sz_zero , sz_one, sz_two, s);
        if(haha){ans=mid+1;   l=mid+1;}
        else{r=mid-1;}
    }
    cout<<ans+2<<endl;
}
