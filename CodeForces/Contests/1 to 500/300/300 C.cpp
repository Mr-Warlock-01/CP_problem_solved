...
void gen_excelent_num(int sum, int pos){
    s.insert(sum);
    if(pos<7){
        gen_excelent_num((sum*10)+a, pos+1);
        gen_excelent_num((sum*10)+b, pos+1);
    }
}

int nCr(int n, int r){
    if(n<r){return 0;}
    int ans= (fact[n]* inv_fact[n-r])%MOD;
...
void solve(int T){
    pre_cal();
    cin>>a>>b>>n;
    gen_excelent_num(a,1);
    gen_excelent_num(b,1);
