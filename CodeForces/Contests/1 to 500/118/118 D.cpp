...
int xxx(int n1, int n2, int k1, int k2, vector<vector<vector<vector<int>>>>&dp){
    if((n1+n2)==0){return 1;}
    if(dp[n1][n2][k1][k2]!=-1){return dp[n1][n2][k1][k2];}

    int x=0;
    if((n1>0) && (k1>0)){ x= xxx(n1-1, n2, k1-1, max_k2, dp);}

    int y=0;
    if((n2>0) && (k2>0)){ y= xxx(n1, n2-1, max_k1, k2-1, dp);}

    return dp[n1][n2][k1][k2]= (x+y)%100000000;
}


void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n1, n2;     cin>>n1>>n2>>max_k1>>max_k2;
    vector<vector<vector<vector<int>>>>dp(n1+1, vector<vector<vector<int>>>(n2+1, vector<vector<int>>(max_k1+1,vector<int>(max_k2+1, -1))));     //4D vector
    int ans=xxx(n1, n2, max_k1, max_k2, dp);
    cout<<ans;
}
