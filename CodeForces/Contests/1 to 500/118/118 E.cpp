...
void bridge(int x, int p, vector<int>graph[], vector<bool>&visit, vector<int>&in, vector<int>&low){
    visit[x]=1; in[x]=low[x]=(timer++);
    for(int i=0 ;i<graph[x].size(); i++){
        if(graph[x][i]==p){continue;}
        else if(visit[graph[x][i]]==0){
            bridge(graph[x][i], x, graph, visit, in, low);
            low[x]= min(low[x], low[graph[x][i]]);
            if(in[x]<low[graph[x][i]]){flag=0;}
        }
        else{
...
void dfs(int x, vector<int>graph[], vector<bool>&visit, map<pair<int,int>,bool>&mp, vector<pair<int,int>>&ans){
    visit[x]=1;
    for(int i=0; i<graph[x].size(); i++){
        if(mp[{x, graph[x][i]}]==0){
            mp[{x, graph[x][i]}]=1;
        ...
        if(visit[graph[x][i]]==0){ dfs(graph[x][i], graph, visit, mp, ans);}
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    bridge(1, -1, graph, visit, in, low);

    for(int i=1; i<=n; i++){if(!visit[i]){flag=0; break;}}
    if(!flag){cout<<flag; return;}

    fill(all(visit),0);
    dfs(1, graph, visit, mp, ans);
