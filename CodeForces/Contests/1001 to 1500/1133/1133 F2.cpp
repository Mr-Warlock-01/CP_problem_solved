...
void union_sets(int a, int b,  vector<int>&parent,  vector<int>&dsu_sz){
    a=find_set(a, parent);
    b=find_set(b, parent);
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>parent(n+1);         //dsu
    vector<int>dsu_sz(n+1,1);       //dsu
    vector<bool>cnt(n+1);           //dis_connected_flag
    vector<int>vertex_with_one;     //every vertex connected with 1 directly
    ...
    for(int i=0; i<m; i++){
        ...
        if(u!=1 && v!=1){
            if(find_set(u, parent) != find_set(v, parent)){
                union_sets(u, v, parent, dsu_sz);
            }
        }
        else{vertex_with_one.pb(max(u,v));}
    }

    for(int i=2; i<=n; i++){
        int a=find_set(i, parent);
        if(!cnt[a]){cnt[a]=1; dis_connected++;}
    }

    if(D<dis_connected || D>vertex_with_one.size()){NO;  return;}

    for(int i=0; i<vertex_with_one.size(); i++){
        int a=find_set(vertex_with_one[i], parent);
        if(a!=1){
            ans.pb({1, vertex_with_one[i]});
            parent[a]=1;
        }
    }

    D-=ans.size();
    for(int i=0; i<parent.size(); i++){parent[i]=i; dsu_sz[i]=1;}
    ...
    for(int i=0; i<vertex_with_one.size(); i++){
        if(D==0){break;}
        if(find_set(1, parent) != find_set(vertex_with_one[i], parent)){
            union_sets(1, vertex_with_one[i], parent, dsu_sz);
            D--;    ans.pb({1, vertex_with_one[i]});
        }
    }
    for(int i=0; i<m; i++){
        int u = edge[i].ff;
