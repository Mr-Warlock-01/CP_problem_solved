...
void pikachu(){         ///MAIN  (DRINK WATER)
    ...
    int sum=0, most_big=0, sec_big=0;

    for(int i=0; i<n; i++){
        sum+=v[i];
        if(v[i]>sec_big){sec_big=v[i];}
        else if(v[i]>most_big){most_big=v[i];}
        if(most_big<sec_big){swap(most_big, sec_big);}
    }

    vector<int>ans;
    for(int i=0; i<n; i++){
        if(v[i]==most_big){
            if((sum-v[i]-sec_big)==sec_big){ans.push_back(i+1);}
        }
        else if((sum-v[i]-most_big)==most_big){ans.push_back(i+1);}
    }
    cout<<ans.size()<<endl;
    cout<<ans;
