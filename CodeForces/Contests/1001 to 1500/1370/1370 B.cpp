...
signed main(){
    ...
    while(tc--){
        ...
        vector<int>n_odd;   vector<int>n_even;
        for(int i=0; i<n; i++){
            if((arr[i]%2)==0){n_even.push_back(i+1);}
            else{n_odd.push_back(i+1);}
        }

        int odd=n_odd.size(), even=n_even.size();
        if((odd%2)==1){odd--;   even--;}
        else if(odd!=0){odd-=2;}
        else{even-=2;}

        for(int i=0; i<odd; i+=2){cout<<n_odd[i]<<" "<<n_odd[i+1]<<endl;}
        for(int i=0; i<even; i+=2){cout<<n_even[i]<<" "<<n_even[i+1]<<endl;}
    }
return 0;
}