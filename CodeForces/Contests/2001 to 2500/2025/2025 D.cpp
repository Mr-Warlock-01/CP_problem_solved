...
int xxx(int i, int stren, int intel){
    if(i>k){return 0;}
    else if(dp[i][stren]!=-1){return dp[i][stren];}
    int zzz =o_pos[i][stren] + o_neg[i][intel];
    int a= xxx(i+1, stren+1, intel);
    int b= xxx(i+1, stren, intel+1);
    return dp[i][stren]= zzz+max(a,b);
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<=k; i++){
        for(int j=1; j<=k; j++){
            o_pos[i][j] = o_pos[i][j]+o_pos[i][j-1];
            o_neg[i][j] = o_neg[i][j]+o_neg[i][j-1];
        }
    }
