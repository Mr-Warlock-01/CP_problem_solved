...
void solve(int T){            ///   (DRINK WATER)
    ...
    for(int i=0; i<m1; i++){cin>>g1[i].ff>>g1[i].ss;}
    for(int i=0; i<m2; i++){cin>>g2[i].ff>>g2[i].ss;}

    
    clean_hobe(n);
    for(int i=0; i<m2; i++){
        if(dsu_2.find_set(g2[i].ff)!=dsu_2.find_set(g2[i].ss)){
            dsu_2.union_sets(g2[i].ff,g2[i].ss);
        }
    }
    ...
    for(int i=0; i<m1; i++){
        int a=g1[i].ff;
        int b=g1[i].ss;
        if(dsu_2.find_set(a)!=dsu_2.find_set(b)){ans++;}
        else{
            if(dsu_1.find_set(a)!=dsu_1.find_set(b)){
                dsu_1.union_sets(a,b);
            }
        }
    }
    ...
    for(int i=1; i<=n; i++){
        int z=dsu_2.find_set(i);
        if(z==i){continue;}
        if(dsu_1.find_set(z)!=dsu_1.find_set(i)){
            ans++;
            dsu_1.union_sets(z,i);
        }
    }
