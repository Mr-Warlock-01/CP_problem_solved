...
void xxx(){
    for(int i=0; i<=n; i++){log_val[i]=log2(arr[i]);}
    x_or[n+1]=0;
    for(int i=n; i>=0; i--){x_or[i]=arr[i]^x_or[i+1];}
...
int find_ans(ll val){
    ...
    while(1){
        int z=log2(new_val);
        ll haha=x_or[check[z][pos]+1];
        ll ok=haha^val;
        if(ok>=arr[pos]){
            new_val=ok^arr[pos];
            if(new_val==0){return pos-1;}
            int zzz=log2(new_val);
            pos=check[zzz][pos-1];
        }
        else{break;}
    }
...
void solve(int T){            ///   (DRINK WATER)
    ...
    arr[0]=(2ll)<<37;   arr[n+1]=0; 
    
    xxx();

    while(q--){
        ll val; cin>>val;
        int ans=n-find_ans(val);
        cout<<ans<<" ";
    }
    cout<<endl;
}
