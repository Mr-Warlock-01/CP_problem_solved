...
int  vis[204][204];
int direction[204][204][4];  // 0=N //1=S //2=E //3=W
...
bool pos_valid( int i, int j){
    if(i<0 || i>=n){return 0;}
    if(j<0 || j>=m){return 0;}
    return 1;
}

int xxx(int i, int j, int flag){
    int mirror_paisi=0;
    while(1){
        if(!pos_valid(i, j)){break;}
        if(vis[i][j]==0){
            vis[i][j]=1;
            if(arr[i][j]!='.'){mirror_paisi++;}
        }
        if(direction[i][j][flag]==1){break;}
        direction[i][j][flag]=1;
        if(arr[i][j]=='.'){
            flag=flag;
        }
        else if(arr[i][j]=='/'){
...
void solve(int T){            ///   (DRINK WATER)
 ...
 //////NNN
    ...
    for(int i=0; i<n; i++){
        ...
        memset(direction, 0, sizeof(direction));
        int z=xxx(i,0,3);
        if(z==mirror_cnt){WWW.pb(i+1);}
    }
    ///////

    int ans=NNN.size() + SSS.size() + EEE.size() + WWW.size();
