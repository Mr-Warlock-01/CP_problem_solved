...
void xxx(int x, int pos){
    ...
    if(pos==0 || pos==3){ans[pos]=a;    ans[pos+1]=b;}
    else{
             if(ans[pos-1]==a){ans[pos+1]=b; swap(ans[pos-1],ans[pos]); }
        else if(ans[pos-1]==b){ans[pos+1]=a; swap(ans[pos-1],ans[pos]); }
        else if(ans[pos]==a){ans[pos+1]=b;}
        else{ans[pos+1]=a;}
    }
}
