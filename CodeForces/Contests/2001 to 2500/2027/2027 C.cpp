...
gp_hash_table<ll, ll,custom_hash>parent;
gp_hash_table<ll, ll,custom_hash>vis;
ll ans=0;

void dfs(int x, vector<ll>graph[]){
    for(int i=0; i<graph[x].size(); i++){
        if(vis[graph[x][i]]==0){
            vis[graph[x][i]]=1;
            ans=max(ans,graph[x][i]);
            dfs(parent[graph[x][i]], graph);
        }
    }
return;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n;  cin>>n;
    vector<ll>v(n);    cin>>v;
    vector<ll>graph[(2*n)+7];
    parent.clear(); vis.clear();
    parent[n]=1;
    ll x=2;    
    ans=n;
    ...
    for(int i=1; i<n; i++){
        ll a=v[i]+i;
        ll b=a+i;
        if(parent[a]==0){parent[a]=x;   x++;}
        if(parent[b]==0){parent[b]=x;   x++;}
        graph[parent[a]].pb(b);
    }
    vis[n]=1;
    dfs(parent[n], graph);
