...
void solve(int T){
    set<pair<int,pair<int,int>>>s1,s0;
    int n;  cin>>n;
    ...
    for(int i=0; i<sq_n; i++){
        for(int j=0; j<sq_n; j++){
            int x=3*i, y=3*j;
            s0.insert({x+y+2,{x+1,y+1}});
            s1.insert({x+y+2,{x+1,y+1}});
            s1.insert({x+y+3,{x+2,y+1}});
            s1.insert({x+y+3,{x+1,y+2}});
            s1.insert({x+y+6,{x+2,y+2}});
        }
    }
    ...
    for(int i=0; i<n; i++){
        if(v[i]==1){
            auto z=*s1.begin();
            cout<<z.ss.ff<<" "<<z.ss.ss<<endl;
            if(s1.find(z)!=s1.end()){s1.erase(z);}
            if(z.ss.ff%3==1 && z.ss.ss%3==1){
                if(s0.find(z)!=s0.end()){s0.erase(z);}
            }
        }
        else{
            auto z=*s0.begin();
