...
void bs_small(int total_sum){
    int t_half=total_sum/2;
    int l=1, r=n;
    int ok,haha;
    while(l<=r){
        int mid=(l+r)/2;
        if(mid%2==0){ok=(mid/2)*((2*k)+mid-1);}
        else{ok=mid*(((2*k)+mid-1)/2);}
        if(ok<=t_half){l=mid+1;}
        else{r=mid-1;}
        haha=total_sum-ok;
        haha=abs(ok-haha);
        ans=min(ans,haha);
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    cin>>n>>k;
    int total_sum;
    if(n%2==0){total_sum=(n/2)*((2*k)+n-1);}
    else{total_sum=n*(((2*k)+n-1)/2);}
