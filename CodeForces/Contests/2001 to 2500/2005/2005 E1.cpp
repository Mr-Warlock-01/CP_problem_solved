...
bool xxx(int pos, int i, int j, vector<int>&a, vector<vector<int>>&b, vector<vector<vector<int>>>&dp){
    if((pos==l) || (i==n) || (j==m)){return 0;}
    if(dp[pos][i][j]!=-1){return dp[pos][i][j];}
    
    bool x=xxx(pos, i+1, j, a, b, dp);
    bool y=xxx(pos, i, j+1, a, b, dp);
    bool z=1;
    if(a[pos]==b[i][j]){z=xxx(pos+1, i+1, j+1, a, b, dp);}
    return dp[pos][i][j] = x|y|(!z);
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<vector<vector<int>>>dp(l+1, vector<vector<int>>(n+1, vector<int>(m+1, -1)));
    
    cin>>a;     cin>>b;
    bool ans=xxx(0,0,0,a,b,dp);
    if(ans){cout<<"T"<<endl;}
    else{cout<<"N"<<endl;}
return;
}
