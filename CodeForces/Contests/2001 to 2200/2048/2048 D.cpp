...
void solve(int T){            ///   (DRINK WATER)
    int n,m;    cin>>n>>m;
    vector<int>a(n),b(m);   cin>>a;     cin>>b;
    ...
    for(int i=0,k=0; i<prob.size(); i++){
        int l=0, r=parti.size()-1;
        int ans=0;
        while(l<=r){
            int mid=(l+r)/2;
            if(parti[mid]<prob[i].ff){l=mid+1;}
            else{r=mid-1; ans=parti.size()-mid;}
        }
        prob[i].ss=ans;
    }
    ...
    for(int i=1; i<=m; i++){
        ...
        z=(i-z)%i;
        if(z==0){z=i;}
        for(int j=z-1; j<prob.size(); j+=i){ans+=prob[j].ss;}
