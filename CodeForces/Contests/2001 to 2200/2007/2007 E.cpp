...
void dfs(int x, vector<int>graph[], vector<int>z_flag[]){
    if(x!=1){z_flag[x].pb(target);}
    if(x==target){target=(target%n)+1;}

    for(int i=0; i<graph[x].size(); i++){
        dfs(graph[x][i], graph, z_flag);
    }
    if(x!=1){z_flag[x].pb(target);}
return;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>z_flag[n+1];
    vector<int>z_count(n+1);
    ...
    for(int TTT=1; TTT<n; TTT++){
        int x,y;    cin>>x>>y;
        w-=y;   rest_sum+=(2*y);
        for(int i=0; i<z_flag[x].size(); i++){
            z_count[z_flag[x][i]]--;
            if(z_count[z_flag[x][i]]==0){z--;}
        }

        int ans=(w*z)+rest_sum;
        cout<<ans<<" ";
    }
    cout<<endl;
return;
