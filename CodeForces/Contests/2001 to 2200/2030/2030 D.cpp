...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    string s;   cin>>s;
    vector<int>l_m_c(n);    //left_most_connesction
    ...
    int cnt=0;
    for(int i=1; i<n; i++){
        if((l_m_c[i-1]==l_m_c[i]) && (s[i-1]=='L')  && (s[i]=='R')){cnt++;}
    }
    ...
    while(q--){
        ...
        if((l_m_c[x-1]==l_m_c[x]) && (s[x-1]=='L')  && (s[x]=='R')){flag--;}
        if((l_m_c[x]==l_m_c[x+1]) && (s[x]=='L')  && (s[x+1]=='R')){flag--;}
