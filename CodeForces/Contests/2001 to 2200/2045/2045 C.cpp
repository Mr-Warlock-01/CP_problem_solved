...
void solve(int T){            ///   (DRINK WATER)
    ...
    for(int i=0; i<26; i++){
        if(alp1[i]==-1){continue;}
        if(alp2[i]==-1){continue;}
        int sz=alp1[i]+s2.size()-alp2[i];
        if(sz<ans_sz){
            ans_sz=sz;
            ans_1=alp1[i];
            ans_2=alp2[i];
    ...
    if(ans_1==-1){cout<<"-1"<<endl;}
    else{
        for(int i=0; i<ans_1; i++){cout<<s1[i];}
        for(int i=ans_2; i<s2.size(); i++){cout<<s2[i];}
    }
}
