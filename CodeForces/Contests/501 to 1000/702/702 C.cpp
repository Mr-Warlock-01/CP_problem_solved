...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n,m;    cin>>n>>m;
    vector<int>city(n), tower(m);   cin>>city;  cin>>tower;     tower.pb(1e18);
    vector<int>ans(n,LONG_MAX);

    int lr=0;
    for(int i=0; i<n; i++){
        int x=abs(city[i]-tower[lr]);
        while(abs(city[i]-tower[lr+1])<=x){
            x=abs(city[i]-tower[lr+1]);  lr++;
        }
        ans[i]=x;
    }
    int ANS=0;
