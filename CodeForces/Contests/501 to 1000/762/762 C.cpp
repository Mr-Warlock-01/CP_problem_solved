...
int BS(int x, vector<int>&v2){
    int l=0, r=v2.size()-1;
    int ans=-1;
    while(l<=r){
        int mid=(l+r)/2;
        if(v2[mid]>x){ans=mid; l=mid+1;}
        else{r=mid-1;}
    }
    return ans;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    string a,b; cin>>a>>b;
    vector<int>v1,v2;   v1.pb(-1);
    int l=0;
    for(int i=0; i<a.size(); i++){
        if(l==b.size()){break;}
        if(a[i]==b[l]){v1.pb(i);   l++;}
    }

    l=b.size()-1;

    for(int i=a.size()-1; i>=0; i--){
        if(l<(v1.size()-1)){break;}
        if(a[i]==b[l]){v2.pb(i);   l--;}
    }
    v2.pb(-1);
    ...
    for(int i=0; i<v1.size(); i++){
        int haha= BS(v1[i], v2)+1;
        if((i+haha)>ans){ans=i+haha; x=i; y=haha-1;}
    }
    if(ans==0){cout<<"-"; return;}
    for(int i=1; i<=x; i++){cout<<a[v1[i]];}
    for(int i=y; i>=0; i--){cout<<a[v2[i]];}
