...
double car_to_polar(double x, double y){
    if(x>0 && y>0){                         return atan(y/x);}
    if(x<0 && y>0){ x*=(-1);                return (PI-atan(y/x));}
    if(x<0 && y<0){ x*=(-1);    y*=(-1);    return (PI+atan(y/x));}
    if(x>0 && y<0){             y*=(-1);    return ((2*PI)-atan(y/x));}
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<n; i++){
        ...
        else if(x==0 && y<0){theta=(3*PI)/2;}
        else{theta=car_to_polar(x, y);}
    ...
    int ans1=angle[0].ss, ans2=angle[n-1].ss;
    double a_flag=angle[n-1].ff-angle[0].ff;
    if(a_flag>PI){a_flag-=(2*PI);     a_flag*=(-1);}
    db(a_flag)
    db(angle)
    for(int i=1; i<n; i++){
        double flag= angle[i].ff- angle[i-1].ff;
        if(flag>PI){flag-=(2*PI);     flag*=(-1);}
        db(flag)
        if(flag<a_flag){a_flag=flag;    ans1=angle[i-1].ss;     ans2=angle[i].ss;}
    }
