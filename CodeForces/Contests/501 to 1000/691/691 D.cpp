...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<pair<int, int>>p(m); cin>>p;
    vector<int>parent(n+1),dsu_size(n+1);
    map<int,priority_queue<int>>mpq;

    for(int i=0; i<n; i++){parent[i]=i; dsu_size[i]=1;}

    for(int i=0; i<m; i++){
        int a= find_par(v[p[i].ff-1], parent);
        int b= find_par(v[p[i].ss-1], parent);
        if(a!=b){
            union_sets( v[p[i].ff-1], v[p[i].ss-1], parent, dsu_size);
        }
    }

    for(int i=1; i<=n; i++){
        int x=find_par(i, parent);
        mpq[x].push(i);
    }

    for(int i=0; i<n; i++){
        int x=find_par(v[i], parent);
        cout<<mpq[x].top()<<" ";
        mpq[x].pop();
    }
