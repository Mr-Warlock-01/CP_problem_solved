...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<n; i++){
        ...
        for(int j=i+1; j<n; j++){
            ...
            if(prime_check(v[i]+v[j])){ok.pb({v[i],v[j]});}
        }
    }
    ...
    if(one_count>0){
        for(int i=0; i<ok.size(); i++){
            if(prime_check(ok[i].ff+1) && prime_check(ok[i].ss+1)){
                cout<<one_count+2<<endl;
                for(int i=0; i<one_count; i++){cout<<"1 ";}
                cout<<ok[i].ff<<" "<<ok[i].ss;
                return;
            }
        }
    }
    ...
    if(ok.size()){
        cout<<"2"<<endl;
        cout<<ok[0].ff<<" "<<ok[0].ss;
        return;
    }
    else{cout<<"1"<<endl<<v[0];}
}
