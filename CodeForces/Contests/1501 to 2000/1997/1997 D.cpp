...
void dfs(int x, vector<int>graph[], queue<int>&qq){
    for(int i=0; i<graph[x].size(); i++){
        dfs(graph[x][i], graph, qq);
    }
    qq.push(x);
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=2; i<=n; i++){graph[parent[i]].pb(i);}
    ...
    while(qq.size()!=1){
        ...
        int flag=LLMax;
        for(int i=0; i<graph[x].size(); i++){
            flag=min(point[graph[x][i]], flag);
        }
        int flag1=(flag+point[x])/2;
        if(flag!=LLMax){point[x]=min(flag, flag1);}
    }

    int ans=LLMax;
    for(int i=0; i<graph[1].size(); i++){
        ans=min(ans,point[graph[1][i]]);
    }
    ans+=point[1];
    cout<<ans<<endl;
}
