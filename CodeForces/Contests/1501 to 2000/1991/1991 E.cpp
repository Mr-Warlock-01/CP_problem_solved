...
void dfs(int x, int clr,  vector<int>graph[], vector<int>&visit){
    if(clr==0){a.push(x);   visit[x]=clr;}
    else{b.push(x); visit[x]=clr;}

    for(int i=0; i<graph[x].size(); i++){
        if(visit[graph[x][i]]==-1){
            dfs(graph[x][i], clr^1, graph, visit);
        }
        else if(visit[graph[x][i]]==clr){is_biper=0;}
    }
}
