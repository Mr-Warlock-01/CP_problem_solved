...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    map<tuple<int, int>, int>   ab_same, bc_same, ac_same;
    map<tuple<int, int, int>, int>  CCC, AAA, BBB;

    int ans=0;
    for(int i=0; i<(n-2); i++){
        ans+= (ab_same[{v[i], v[i+1]}]++) - (CCC[{v[i], v[i+1], v[i+2]}]++);
        ans+= (bc_same[{v[i+1], v[i+2]}]++) - (AAA[{v[i+1], v[i+2], v[i]}]++);
        ans+= (ac_same[{v[i], v[i+2]}]++) - (BBB[{v[i], v[i+2], v[i+1]}]++);
    }
    cout<<ans<<endl;
}
