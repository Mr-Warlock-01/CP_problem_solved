...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>far(n);      //farest node can in 1 move
    vector<int>reach(n);    //min moves need to reach i node 
    vector<bool>ans(n,1);
    ...
    for(int i=2; i<n; i++){
        int x=reach[i-1];
        for(int u: rev_graph[i]){x=min(x,reach[u]);}
        reach[i]=x+1;
    }
