...
bool s_check(string s1, string s2){
    int sz=s1.size();
    int ans=0;
    for(int i=0; i<sz; i++){
        if(s1[i]!=s2[i]){ans++;}
...
void pikachu(){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<v.size(); i++){
        ...
        if(flag){
            if(s1==s2){
                if(m[s1]==(n/v[i])){flag=1;}
                else if((m[s1]+1)==(n/v[i])){
                    if(s_check(s1,s3)){flag=1;}
                    else{flag=0;}
                }
                else{flag=0;}
            }
            else if( (m[s1]==1) || (m[s2]==1) ){
                if((m[s1]+m[s2])==(n/v[i])){
                    if(s_check(s1,s2)==1 && s3=="" ){flag=1;}
                    else{flag=0;}
                }
                else{flag=0;}
            }
