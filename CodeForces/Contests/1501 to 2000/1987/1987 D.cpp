...
int xxx(int x, int can_remove, vector<int>&v, vector<vector<int>>&dp){
    if(x==v.size()){return 0;}
    if(dp[x][can_remove]!=LLMax){return dp[x][can_remove];}
    int a,b=LLMax;
    a= xxx(x+1, can_remove+1, v, dp)+1;
    if(can_remove>=v[x]){
        b=xxx(x+1, can_remove-v[x], v, dp);
    }
    return dp[x][can_remove]=min(a,b);
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n;  cin>>n;
    vector<int>v_1(n);    cin>>v_1;
    vector<int>v;
    sort(all(v_1));    v_1.pb(-1);
    ...
    db(v)
    int sz=v.size()+1;
    vector<vector<int>>dp(sz, vector<int>(sz, LLMax));

    int ans=xxx(0, 0, v, dp);
    cout<<ans<<endl;
}
