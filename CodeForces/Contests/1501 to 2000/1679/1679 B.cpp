...
int powMod(int x, int n, int M){int res=1; while(n){if(n%2){res=(res*x)%M;} x=(x*x)%M; n>>=1;} return res;}
...
void build(int arr[], int tree[], int node, int st, int en){
...
int lazy_query(int tree[], int lazy[], int node, int st, int en, int l, int r){
    if(lazy[node]!=0){
...
void lazy_update(int tree[], int lazy[], int node, int st, int en, int l, int r, int val){
    if(lazy[node]!=0){
        tree[node]=((en-st+1)*lazy[node]);
        if(st!=en){ lazy[2*node]=lazy[node];   lazy[2*node+1]=lazy[node];}
        lazy[node]=0;
...
void pikachu(){         ///MAIN  (DRINK WATER)
    ...
    while(q--){
        ...
        if(z==1){
            int x;  cin>>x;
            int i;  cin>>i;
            lazy_update(tree, lazy,1, 0, n-1, x-1, x-1, i);
            cout<<lazy_query(tree, lazy,1, 0, n-1, 0, n-1)<<endl;
        }
        else{
            int x;  cin>>x;
            lazy_update(tree, lazy,1, 0, n-1, 0, n-1, x);
            cout<<lazy_query(tree, lazy,1, 0, n-1, 0, n-1)<<endl;
        }
    }
}
