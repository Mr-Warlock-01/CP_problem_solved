...
signed main(){
    ...
    while(tc--){
        ...
        //int sma=0;
        int sz=s.length();
        bool flag[sz];  //for(int i=0; i<sz; i++){flag[i]=0;}
        vector<int>CA;
        vector<int>SM;

        for(int i=0; i<sz; i++){
            if(s[i]>='A' && s[i]<='Z' && s[i]!='B'){        flag[i]=1;     CA.push_back(i);    }
            else if(s[i]>='a' && s[i]<='z' && s[i]!='b'){   flag[i]=1;     SM.push_back(i);    }
            else if(s[i]=='B' && CA.size()==0){flag[i]=0;                       }
            else if(s[i]=='b' && SM.size()==0){flag[i]=0;                       }
            else if(s[i]=='B'){flag[i]=0;   flag[CA[CA.size()-1]]=0;    CA.pop_back(); }
            else if(s[i]=='b'){flag[i]=0;   flag[SM[SM.size()-1]]=0;    SM.pop_back();   }
