...
int coprimeCount(int n){int ans=n; for(int i=2; i*i<=n; i++){ if(n%i==0){ans/=i; ans*=(i-1); while(n%i==0){n/=i;}}}if(n>1){ ans/=n; ans*=(n-1);} return ans;}
...
void pikachu(){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<n; i++){
        ...
        db(x);
        if(x==0){ans=i; break;}
        else if(x==1 && flag==0){flag=1;}
        else if(x==1 && flag==1){ans=i; break;}
    }
    cout<<ans<<endl;
}
