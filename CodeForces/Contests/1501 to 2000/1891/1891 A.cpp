...
signed main(){
    ...
    while(t--){
        int n;  cin>>n;
        int a[21];
        for(int i=0; i<21; i++){a[i]=INT_MAX;}
        a[0]=0;
        for(int i=1; i<=n; i++){cin>>a[i];}

        if((a[3]>a[4]) || (a[5]>a[6]) || (a[6]>a[7]) || (a[7]>a[8]) ||(a[9]>a[10])){
            cout<<"NO"<<endl;
        }
        else if((a[10]>a[11]) || (a[11]>a[12]) || (a[12]>a[13]) || (a[13]>a[14]) ||(a[14]>a[15])){
            cout<<"NO"<<endl;
        }
        else if((a[15]>a[16])||(a[17]>a[18]) || (a[18]>a[19]) || (a[19]>a[20])) {
            cout<<"NO"<<endl;
        }
        else{cout<<"YES"<<endl;}
