...
void pikachu(){         ///MAIN  (DRINK WATER)
    ...
    l_to_r[1]=1;
    for(int i=2; i<n; i++){
        if( (v[i-1]-v[i-2]) > (v[i]-v[i-1]) ){ l_to_r[i] = (l_to_r[i-1] + 1 ); }
        else{ l_to_r[i] = ( l_to_r[i-1]+ v[i] - v[i-1] );}
    }

    r_to_l[n-2]=1;
    for(int i=n-3; i>=0; i--){
        if( (v[i+2]-v[i+1]) > (v[i+1]-v[i]) ){ r_to_l[i] = (r_to_l[i+1] + 1 ); }
        else{ r_to_l[i] = ( r_to_l[i+1]+ v[i+1] - v[i] );}
    }
    ...
    while(m--){
        cin>>a>>b;
        if(a<b){ ans = l_to_r[b-1] - l_to_r[a-1];}
        else{ ans = r_to_l[b-1] - r_to_l[a-1];}
        cout<<ans<<endl;
    }
}
