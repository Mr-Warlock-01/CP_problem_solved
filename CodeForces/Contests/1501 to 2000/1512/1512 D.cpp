...
signed main(){
    ...
    while(t--){
        int n;  cin>>n;
        int z=n+2;
        int BB[z];     for(int i=0; i<z; i++){cin>>BB[i];}
        sort(BB,BB+z);

        int Last_1=BB[z-1];
        int Last_2=BB[z-2];
        int sum=0;
        for(int i=0 ; i<z; i++){sum+=BB[i];}
        int haha=sum-(Last_1+Last_2);
        int sum_index=-1;
        int added_index=-1;
        ...
        if(sum_index!= -1 & added_index!= -1){
            for(int i=0 ; i<z; i++){
                if(i==sum_index || i==added_index){continue;}
                cout<<BB[i]<<" ";
            }
        }
        else{cout<<"-1";}
