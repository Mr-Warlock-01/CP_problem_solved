...
BS_l(vector<int>&sum, int x){
    int ans=sum.size()-1;
    int ll=0, rr=sum.size()-1,mid;
    while(ll<=rr){
        mid=(ll+rr)/2;
        if(sum[mid]==x){ans=mid;  rr=mid-1;}
        else if(sum[mid]>x){rr=mid-1;}
        else{ll=mid+1;}
    }
    return (ans+1);
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    for(int i=0; i<n; i++){
        ...
        int x=v[i];
        while(v[i]!=0){
            if(v[i]%2==1){one[i].push_back(flag);}
            v[i]/=2;
            flag++;
        }
    }
    ...
    while(l<=r){
        ...
        for(int i=mid; i<n; i++){
            ...
            for(int j=0; j<one[i-mid].size();j++){
                ok[one[i-mid][j]]--;
                if(ok[one[i-mid][j]]==0){haha=0; break;}
            }
        }
        if(haha){ans=mid;   r=mid-1;}
        else{l=mid+1;}
    }
    cout<<ans<<endl;
}
