...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    vector<int>z(n);    for(int i=0; i<n; i++){cin>>z[i];}
    int l=-1,r=n;
    if(z[n-1]<0){l=n-1;}
    else if(z[0]>0){r=0;}
    else{
    ...
    int position=0, fire_rem=k;
    bool ans=1, l_r;
    while( l>=0 || r<n){
        if(l<0){l_r=1;}
        else if(r==n){l_r=0;}
        else{
            if(abs(z[l]) < abs(z[r])){l_r=0;}
            else{l_r=1;}
        }

        if(l_r==0){
            if(position>= abs(z[l])){ans=0; break;}
            position+= (v[l]/k);
            fire_rem-=(v[l]%k);
            if(fire_rem==k){fire_rem=0; position--;}
            if(fire_rem<0){fire_rem+=k; position++;}
            if(position>= abs(z[l])){ans=0; break;}
            l--;
        }
        else{
            if(position>= abs(z[r])){ans=0; break;}
            position+= (v[r]/k);
            fire_rem-=(v[r]%k);
            if(fire_rem==k){fire_rem=0; position--;}
            if(fire_rem<0){fire_rem+=k; position++;}
        ...
        if(fire_rem==0){position++; fire_rem=k;}
    }
