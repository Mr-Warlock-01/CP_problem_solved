...
signed main(){
    ...
    int a;  cin>>a;
    int a_check=0;
    int x=4;        //smallest joke
    while(a_check<a+1){
        vector<int>v;
        int z=x;
        int sum_ini=0;
        int sum_prime=0;
        for(int i=0; i<5; i++){ sum_ini+=(z%10);    z/=10;}
        z=x;

        for(int i=2; z!=1;){
            if(z%i==0){ v.push_back(i); z/=i;}
            else{i++;}
        }
        int sz=v.size();
        if(sz==1){x++;continue;}
        ...
        for(int i=0; i<sz; i++){
            int haha=v[i];
            int lol=0;
            for(int j=0; j<5; j++){ lol+=(haha%10); haha/=10;}
            sum_prime+=lol;
        }
        if(sum_prime==sum_ini){a_check++;}
        if(a==a_check){cout<<x; return 0;}
        x++;
    }
