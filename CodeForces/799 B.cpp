...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n;  cin>>n;
    vector<int>v(n),a(n),b(n);   cin>>v; cin>>a; cin>>b;


    ordered_set<int>os;     for(int i=0; i<n; i++){os.insert(v[i]);}

    priority_queue<int>z[3];
    for(int i=0; i<n; i++){
        z[a[i]-1].push(-v[i]);
        z[b[i]-1].push(-v[i]);
    }
    ...
    while(q--){
        ...
        while(1){
            if(!z[x-1].empty()){
                if((os.order_of_key(-z[x-1].top()+1)-os.order_of_key(-z[x-1].top()))!=0){
                    cout<<-z[x-1].top()<<" ";
                    os.erase(-z[x-1].top());
                    z[x-1].pop();
                    break;
