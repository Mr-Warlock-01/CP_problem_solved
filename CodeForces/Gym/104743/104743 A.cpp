...
signed main(){
    ...
    while(tc--){
        ...
        vector<int>v_1(n);  for(int i=0; i<n; i++){v_1[i]=(v[i]&1);}
        vector<int>v_2(n);  for(int i=0; i<n; i++){v_2[i]=(v[i]&2);}

        sort(rall(v));
        sort(rall(v_1));
        sort(rall(v_2));

        if(v[0]==0){cout<<"0"<<endl;    continue;}
        else if(v_1[0]==0){cout<<"1"<<endl; continue;}
        else if( (v_2[0]==0)  && (k>1)  ){cout<<"1"<<endl;  continue;}
        ...
        for(int i=4; i<=k; i*=2){
            ...
            for(int j=0; j<n; j++){
                haha=v[j]&i;
                if(haha!=0){flag=0; break;}
            }
            if(flag==1){break;}
        }

        if(flag==1){cout<<"1"<<endl;    continue;}
        else if(k>1){cout<<"2"<<endl;    continue;}
