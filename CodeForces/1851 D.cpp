...
void pikachu(){         ///MAIN  (DRINK WATER)
    ...
    vector<int>v(n);
    int big_than_n_count=0, big_n_value, zero_count=n, two_count=0;

    for(int i=1; i<n; i++){
        int x=arr[i]-arr[i-1];
        if(x>n){big_than_n_count++; big_n_value=x;}
        else{
            v[x-1]++;
            if(v[x-1]==1){zero_count--;}
            else if(v[x-1]>1){two_count++;}
        }
    }
    if(zero_count==1){YES;}
    else if(big_than_n_count>1){NO;}
    else if(big_than_n_count==1){
        if(zero_count!=2){NO;}
        else{
            int zero;
            for(int i=0; i<n; i++){if(v[i]==0){zero=i+1;    v[i]=1;     break;}}
            int lol=big_n_value - zero-1;
            if(lol>=0 && lol<n){
                if(v[lol]==0){YES;}
                else{NO;}
            }
        ...
        if(two_count==1 && zero_count==2){
            int two,zero;
            for(int i=0; i<n; i++){if(v[i]==2){two=i+1; break;}}
            for(int i=0; i<n; i++){if(v[i]==0){zero=i+1;    v[i]=1;     break;}}
            int lol=two - zero - 1;
