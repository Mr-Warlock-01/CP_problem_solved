...
signed main(){
    ...
    cin>>tc;
    while(tc--){
        double n;   int p,l,t;  cin>>n>>p>>l>>t;
        int t_unlock=ceil(n/7);
        int LL=1, RR=n, mid, ans=n, point_check;
        while(LL<=RR){
            mid=((RR-LL)/2)+LL;
            point_check=(t*(min(2*mid,t_unlock)))+(mid*l);
            if(point_check>=p){ans=min(ans,mid);    RR=mid-1;}
            else{LL=mid+1;}
        }
        ans=n-ans;
        cout<<ans<<endl;
