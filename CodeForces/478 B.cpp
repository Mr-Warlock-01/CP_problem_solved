...
signed main(){
    ...
    while(tc--){
        int n,m;  cin>>n>>m;
        //n=total mate      m=total team
        int low_mate = n/m;
        int high_mate = low_mate + 1;
        int high_team = n%m;
        int low_team= m - high_team;
        int ans_1= low_team*((low_mate * (low_mate-1))/2);
        int ans_2= high_team* ((high_mate *(high_mate-1))/2);
        int ans_min=ans_1+ans_2;

        int ans_max= (((n+1)-m)*(n-m))/2;
