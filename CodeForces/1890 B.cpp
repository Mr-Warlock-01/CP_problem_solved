...
signed main(){
    ...
    while(TT --){
        ...
        if(flag_s==1){cout<<"Yes"<<endl;    continue;}
        else if(one!=0 & zero!=0){cout<<"No"<<endl;    continue;}

        if(m%2==1){
            for(int i=1; i<m; i++){
                if(t[i]==t[i-1]){   flag_t=0;   break;}
            }
        }
        else{flag_t=0;}
        if(flag_t==0){cout<<"No"<<endl;    continue;}
        if(one>0 & t[0]=='0'){cout<<"Yes"<<endl;    continue;}
        else if(zero>0 & t[0]=='1'){cout<<"Yes"<<endl;    continue;}
