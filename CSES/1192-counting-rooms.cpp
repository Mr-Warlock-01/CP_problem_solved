...
void bfs(int y, int x, int n, int m, vector<vector<char>>&v, vector<vector<bool>>&visit){
    queue<pair<int,int>>qq;
    qq.push({y,x});    visit[y][x]=1;
    while(!qq.empty()){
        int yy=qq.front().ff;
        int xx=qq.front().ss;
        qq.pop();
        if((xx>0) && (visit[yy][xx-1]==0) && (v[yy][xx-1]=='.')){
            visit[yy][xx-1]=1;  qq.push({yy,xx-1});
        }
        if((yy>0) && (visit[yy-1][xx]==0) && (v[yy-1][xx]=='.')){
            visit[yy-1][xx]=1;  qq.push({yy-1,xx});
        }
        if(((xx+1)<m) && (visit[yy][xx+1]==0) && (v[yy][xx+1]=='.')){
            visit[yy][xx+1]=1;  qq.push({yy,xx+1});
        }
        if(((yy+1)<n) && (visit[yy+1][xx]==0) && (v[yy+1][xx]=='.')){
            visit[yy+1][xx]=1;  qq.push({yy+1,xx});
        }
    }
}
