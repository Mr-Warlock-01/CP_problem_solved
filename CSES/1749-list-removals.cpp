...
void build(int node, int st, int en){
    if(st==en){treee[node].ff = v[st].ff;    treee[node].ss=v[st].ss;     return;}
    int mid=((en-st)/2)+st;
    build(2*node, st, mid);
    build(2*node+1, mid+1, en);
    treee[node].ss=treee[2*node].ss+treee[2*node+1].ss;
    return;
}



int query(int node, int st, int en, int posi){
    if(st==en){treee[node].ss=0; return treee[node].ff;}
    int mid=((en-st)/2)+st;
    int x;
    if(treee[2*node].ss>= posi){x=query(2*node, st, mid, posi);}
    else{x=query(2*node+1, mid+1, en, posi-treee[2*node].ss );}
    treee[node].ss=treee[2*node].ss + treee[2*node+1].ss;
    return x;
}
