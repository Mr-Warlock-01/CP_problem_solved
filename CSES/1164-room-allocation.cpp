...
void build(vector<int>&tree, int node, int st, int en){
    if(st==en){tree[node]=1;    return;}
    int mid=((en-st)/2)+st;
    build(tree, 2*node, st, mid);
    build(tree, 2*node+1, mid+1, en);
    tree[node]=tree[2*node]+tree[2*node+1];
    return;
}

void in(vector<int>&tree, int node, int st, int en, int room){
    if(st==en){tree[node]=0;    num=st; return;}
    int mid=((en-st)/2)+st;
    if(room<=tree[2*node]){in(tree,2*node, st, mid, room);}
    else{in(tree,2*node+1, mid+1, en, room-tree[2*node]);}
    tree[node]=tree[2*node]+tree[2*node+1];
    return;
}

void out(vector<int>&tree, int node, int st, int en, int ind){
    if(st==en){ tree[node]=1;   return;}
    int mid=((en-st)/2)+st;
    if(st<=ind && mid>=ind){    out(tree, 2*node,st, mid, ind);}
    else{                       out(tree, 2*node+1, mid+1, en, ind);}
    tree[node]=tree[2*node] + tree[2*node+1];
    return;
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    int n;  cin>>n;
    vector<pair<int,pair<int,int>>>v(2*n);        //0 in  //1=left
    vector<int>tree(4*n);
    vector<int>flag(n);
    vector<int>ans(n);
    for(int i=0; i<2*n; i++){cin>>v[i].ff;    if((i%2)==1){v[i].ss.ff=1;};    v[i].ss.ss=i/2;}
    ...
    for(int i=0; i<2*n; i++){
        if(v[i].ss.ff==0){
            in(tree, 1, 1, n, 1);   flag[v[i].ss.ss]=num;
            ans[v[i].ss.ss]=num;    MMM=max(MMM,num);
        }
        else{
            out(tree, 1, 1, n, flag[v[i].ss.ss]);
        }
    }
