...
const int N=200005;
int arr[N];
int tree_r[4*N], tree_l[4*N];

int n,q;



void build_r(int node, int st, int en){
    if(st==en){  tree_r[node] = arr[st]+st; return;}
    int mid=((en-st)/2)+st;
    build_r(2*node, st, mid);
    build_r(2*node+1, mid+1, en);
    tree_r[node]=min(tree_r[2*node],tree_r[2*node+1]);
return;
}

void build_l(int node, int st, int en){
    if(st==en){  tree_l[node] = arr[st]-st; return;}
    int mid=((en-st)/2)+st;
    build_l(2*node, st, mid);
    build_l(2*node+1, mid+1, en);
    tree_l[node]=min(tree_l[2*node],tree_l[2*node+1]);
    return;
}

void update_r(int node, int st, int en, int ind, int val){
    if(st==en){tree_r[node]=val+ind;   return;}
    int mid=((en-st)/2)+st;
    if(st<=ind && mid>=ind){    update_r(2*node,st, mid, ind, val);}
    else{                       update_r(2*node+1, mid+1, en, ind, val);}
    tree_r[node]=min(tree_r[2*node],tree_r[2*node+1]);
    return;
}

void update_l(int node, int st, int en, int ind, int val){
    if(st==en){tree_l[node]=val-ind;   return;}
    int mid=((en-st)/2)+st;
    if(st<=ind && mid>=ind){    update_l(2*node,st, mid, ind, val);}
    else{                       update_l(2*node+1, mid+1, en, ind, val);}
    tree_l[node]=min(tree_l[2*node], tree_l[2*node+1]);
    return;
}

ll query_r(int node, int st, int en, int l, int r){
    if(st>r || en<l){return LLMax;}
    else if(st>=l && en<=r){return tree_r[node];}
    int mid=((en-st)/2)+st;
    ll q1= query_r(2*node, st, mid, l, r);
    ll q2= query_r(2*node+1, mid+ 1, en, l, r);
    return min(q1,q2);
}

ll query_l(int node, int st, int en, int l, int r){
    if(st>r || en<l){return LLMax;}
    else if(st>=l && en<=r){return tree_l[node];}
    int mid=((en-st)/2)+st;
    ll q1= query_l(2*node, st, mid, l, r);
    ll q2= query_l(2*node+1, mid+ 1, en, l, r);
    return min(q1,q2);
}
...
void solve(int T){            ///   (DRINK WATER) 
    ...
    while(q--){
        ...
        if(a==1){
            ...
            update_r(1, 0, n-1, idx-1, val);
            update_l(1, 0, n-1, idx-1, val);
            //db(val);
        }
        else{
            int idx;    cin>>idx;
            ll a=query_r(1, 0, n-1, idx-1,n-1)-(idx-1);
            ll b=query_l(1, 0, n-1, 0,idx-1)+(idx-1);
            cout<<min(a,b)<<endl;
        }
    }
return; 
