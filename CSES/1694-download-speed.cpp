...
int bfs(int t,  vector<vector<int>>&capacity, vector<vector<int>>&edge, vector<int>&parent){
    ...
    while(!qq.empty()){
        ...
        qq.pop();
        for(int x: edge[u]){
            if(capacity[u][x]!=0 && parent[x]== -1){
                parent[x]=u;
                int dd=min(d, capacity[u][x]);
                if(x==t){return dd;}
                qq.push({x, dd});
            }
        }
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    while(m--){
        ...
        if(a==n){continue;}
        capacity[a-1][b-1]+=c;
        edge[a-1].pb(b-1);  edge[b-1].pb(a-1);
    }

    int max_flow=0;
    int new_flow;

    while(new_flow = bfs(n-1, capacity, edge, parent)){
        max_flow+=new_flow;
        int cur=n-1;
        while(cur!=0){
            int prev = parent[cur];
