...
int n;
void dfs(int x, vector<int>graph[], vector<int>&visit){
    if(x==n){visit[x]=1;}
    for(int i=0; i<graph[x].size(); i++){
        if(visit[graph[x][i]]==LLMin){
            dfs(graph[x][i], graph, visit);
        }
        visit[x]=max(visit[x], visit[graph[x][i]]+1);
    }
}
...
void pikachu(int T){         ///MAIN  (DRINK WATER)
    ...
    if(visit[n]!=1){cout<<"IMPOSSIBLE";}
    else{
        int x = visit[1];     cout<<x<<endl;
        int z=1;
        while(x--){
            cout<<z<<" ";
            for(int i=0; i<graph[z].size(); i++){
                if(visit[graph[z][i]]==x){z=graph[z][i]; break;}
            }
        }
    }
