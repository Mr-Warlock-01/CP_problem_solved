...
bool BS(int x){
    ...
    for(int i=1; i<=n; i++){
        for(int j=1; j<=n; j++){
            check[i][j]+=(check[i][j-1]+check[i-1][j]-check[i-1][j-1]);
        }
    }
    ...
    for(int i=k; i<=n; i++){
        for(int j=k; j<=n; j++){
            int cnt=check[i][j]-check[i][j-k]-check[i-k][j]+check[i-k][j-k];
            if(cnt<=0){return 1;}
        }
    }
    return 0;
