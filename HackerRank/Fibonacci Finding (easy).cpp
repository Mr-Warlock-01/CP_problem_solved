...
void pikachu(){         ///MAIN  (DRINK WATER)
    ...
    int mat[2]={0,1};   cin>>mat[0]>>mat[1];
     cin>>n;
    if(n==0){cout<<mat[0]<<endl; return;}
    else if(n==1){cout<<mat[1]<<endl; return;}
    while(n){
        if(n%2){
            z[0][0]=I[0][0] , z[1][0]=I[1][0] , z[0][1]=I[0][1] , z[1][1]=I[1][1] ;
            I[0][0]=((z[0][0]*M[0][0])+(z[0][1]*M[1][0]))%MOD;
            I[0][1]=((z[0][0]*M[0][1])+(z[0][1]*M[1][1]))%MOD;
            I[1][0]=((z[0][1]*M[0][0])+(z[1][1]*M[0][1]))%MOD;
            I[1][1]=((z[0][1]*M[0][1])+(z[1][1]*M[1][1]))%MOD;
        }
        n/=2;
        z[0][0]=M[0][0] , z[1][0]=M[1][0] , z[0][1]=M[0][1] , z[1][1]=M[1][1] ;

        M[0][0]=((z[0][0]*z[0][0])+(z[0][1]*z[1][0]))%MOD;
        M[0][1]=((z[0][0]*z[0][1])+(z[0][1]*z[1][1]))%MOD;
        M[1][0]=((z[0][1]*z[0][0])+(z[1][1]*z[0][1]))%MOD;
        M[1][1]=((z[0][1]*z[0][1])+(z[1][1]*z[1][1]))%MOD;
    }
    cout<<((mat[0]*I[0][0])+(mat[1]*I[1][0]))%MOD<<endl;
}
