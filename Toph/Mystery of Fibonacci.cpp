// httpstoph.copmystery-of-fibonacci
...
vector<vector<int>> matrix_mul(vector<vector<int>>P1, vector<vector<int>>P2, ll M){
    int n=P1.size();
    int m=P2[0].size();
    int sz=P2.size();
...
vector<vector<int>> matrix_pow(vector<vector<int>>P, ll n, ll M){
    int sz=P.size();
...
void  solve(int T){
    int n;  cin>>n;
    vector<vector<int>>Pat = {{0},{1}};
    vector<vector<int>>Seq = {{0,1},{1,1}};

    Seq=matrix_pow(Seq,n, MOD);
    Pat=matrix_mul(Seq,Pat, MOD);
    cout<<Pat[0][0];
}
