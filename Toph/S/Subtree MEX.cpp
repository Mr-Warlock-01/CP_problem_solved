...
void update(int idx, int out, int in){
    if(out==in){return;}
    val[idx]=in;
    while(1){
        if(idx==-1){break;}
...
int query(int idx){
    ...
    int ansss=0;
    int l=0; int r=os[idx].size();
    while(l<=r){
        int mid=((r-l)/2)+l;
        int z=os[idx].order_of_key(mid);
        if(z==mid){ansss=mid; l=mid+1;}
        else{r=mid-1;}
    }
    //return ansss;
return ans[idx]=ansss;
}
