// https://toph.co/p/jumping-frog-ii
...
void solve(int T){
    ...
    for(int i=1; i<=n; i++){
        ...
        if(arr[i]!=0){
            lazy_update(1,1,n, i,i+arr[i]-1,1);
        }
    }
    ...
    while(q--){
        ...
        if(t==1){
            ...
            s=min(s,n-p+1);
            if(arr[p]>s){
                lazy_update(1,1,n,p+s,p+arr[p]-1, -1);
            }
            else if(arr[p]<s){
                lazy_update(1,1,n, p+arr[p], p+s-1, 1);
            }
            arr[p]=s;
        }
        else{
