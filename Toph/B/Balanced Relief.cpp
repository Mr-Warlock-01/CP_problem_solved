// https://toph.co/p/balanced-relief
...
int xxx(vector<ll>&v, ll x){
    int l=0, r=v.size()-1;
    int ans=1e8;
    while(l<=r){
        int mid=(l+r)/2;
        if(x+v[mid]>=low){ans=mid;  r=mid-1;}
        else{l=mid+1;}
    }
    return ans;
}
...
int yyy(vector<ll>&v, ll x){
    int l=0, r=v.size()-1;
    int ans=-100;
    while(l<=r){
        int mid=(l+r)/2;
        if((x+v[mid])<=high){ans=mid;  l=mid+1;}
        else{r=mid-1;}
    }
    return ans;
}
...
void solve(int T){            ///   (DRINK WATER)
    ...
    vector<ll>male(n);      cin>>male;
    vector<ll>female(m);    cin>>female;
    ...
    ll ans=0;
    for(int i=0; i<n; i++){
        ll l=xxx(female, male[i]);
        ll r=yyy(female, male[i]);
        if(l<=r){ans+=(r-l+1);}
    }
    cout<<ans<<endl;
}
