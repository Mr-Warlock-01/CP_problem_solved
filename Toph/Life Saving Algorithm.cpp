// https://toph.co/p/life-saving-algorithm
...
void f(int i, int j){
    ...
    for(int z=0; z<8; z++){
        ...
        int y=j+dy[z];
        if(x<0 || y<0){continue;}
        if((x<n) && (y<n) && (vis[x][y]==0) && (v[x][y]=='#')){f(x,y);}
    }
}
...
void d(int i, int j){
    ...
    if(danger_found==0){
        for(int z=0; z<8; z++){
            ...
            int y=j+dy[z];
            if(x<0 || y<0){continue;}
            if((x<n) && (y<n) && (v[x][y]=='#')){danger_found=1; break;}
        }
    }
    ...
    for(int z=0; z<4; z++){
        ...
        int y=j+dy[z];
        if(x<0 || y<0){continue;}
        if((x<n) && (y<n) && (vis[x][y]==0) && v[x][y]=='*'){d(x,y);}
    }
}
