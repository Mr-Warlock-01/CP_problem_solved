...
int arr[N];
vector<int>diablo(4*N);

void build(int node, int st, int en){
    if(st==en){diablo[node]=1;return;}
    int mid=(st+en)/2;
    build(2*node, st, mid);
    build(2*node+1, mid+1, en);
    diablo[node]=diablo[2*node]+diablo[2*node+1];
return;
}

void update(int node, int st, int en, int idx){
    if(st==en){ diablo[node]=0;   return;}
    int mid=(st+en)/2;
...
int query(int node,int st, int en, int pos){
    ...
    int mid=(st+en)/2;
    int ans;
    if(pos<=diablo[2*node]){ans=query(2*node, st, mid, pos);}
    else{ans=query(2*node+1, mid+1, en, pos-diablo[2*node]);}
return ans;
}
...
void solve(int T){ 
    ...
    while(q--){
        ...
        x++;
        int L=-1, R=-1;
        if(x>1){L=query(1, 0, n-1, x-1);}
        if(x<sz){R=query(1, 0, n-1, x+1);}
        int idx=query(1, 0, n-1, x);
        arr[idx]=y;
        if(R!=-1 && arr[R]==arr[idx]){
            update(1, 0, n-1, R);
            sz--;
        }
        if(L!=-1 && arr[L]==arr[idx]){
            update(1, 0, n-1, L);
            sz--;
        }
        cout<<sz<<endl;
