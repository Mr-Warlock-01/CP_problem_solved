// https://toph.co/p/festive-shuffling
...
int xxx(int i, int j, int cnt0, int cnt1) {
    if(i>j){return 0;}
    if(i==j){return 1;}
    if(dp[i][j][cnt1]!=-1){return dp[i][j][cnt1];}
    
    int ans=0;
    if((is_special[i]==1) && (is_special[j]==1)){
        if(cnt1>1){ans=max(ans, 2+ xxx(i+1, j-1, cnt0, cnt1-2));}
        if(cnt0>1){ans=max(ans, 2+ xxx(i+1, j-1, cnt0-2, cnt1));}
        if(cnt0!=0){
            ans=max(ans, xxx(i, j-1, cnt0-1, cnt1));
            ans=max(ans, xxx(i+1, j, cnt0-1, cnt1));
        }
        if(cnt1!=0){
            ans=max(ans, xxx(i, j-1, cnt0, cnt1-1));
            ans=max(ans, xxx(i+1, j, cnt0, cnt1-1));
        }
    }

    if((is_special[i]==0) && (is_special[j]==0)){
        if(s[i]==s[j]){ans=max(ans, 2+xxx(i+1, j-1, cnt0, cnt1));}
        ans=max(ans, xxx(i+1, j, cnt0, cnt1));
        ans=max(ans, xxx(i, j-1, cnt0, cnt1));
    }


    if((is_special[i]==1) && (is_special[j]==0)){
        if(cnt0>0){
            if(s[j]=='0'){ans=max(ans, 2 +xxx(i+1, j-1, cnt0-1, cnt1));}
            else{
                ans=max(ans,xxx(i+1, j, cnt0-1, cnt1));
                ans=max(ans,xxx(i, j-1, cnt0, cnt1));
            }
        ...
        if(cnt1>0){
            if(s[j]=='1'){ans=max(ans, 2 +xxx(i+1, j-1, cnt0, cnt1-1));}
            else{
                ans=max(ans,xxx(i+1, j, cnt0, cnt1-1));
                ans=max(ans,xxx(i, j-1, cnt0, cnt1));
            }
    ...
    if((is_special[i]==0) && (is_special[j]==1)){
        if(cnt0>0){
            if(s[i]=='0'){ans=max(ans, 2 +xxx(i+1, j-1, cnt0-1, cnt1));}
            else{
                ans=max(ans,xxx(i, j-1, cnt0-1, cnt1));
                ans=max(ans,xxx(i+1, j, cnt0, cnt1));
            }
        ...
        if(cnt1>0){
            if(s[i]=='1'){ans=max(ans, 2 +xxx(i+1, j-1, cnt0, cnt1-1));}
            else{
                ans=max(ans,xxx(i, j-1, cnt0, cnt1-1));
                ans=max(ans,xxx(i+1, j, cnt0, cnt1));
            }
